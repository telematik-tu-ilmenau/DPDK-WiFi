...
void ath10k_htt_tx_dec_pending_bulk(struct ath10k_htt *htt, int num_msdus)
{
    lockdep_assert_held(&htt->tx_lock);

    htt->num_pending_tx -= num_msdus;
    // TODO: customize and reactivate if needed
    if (htt->num_pending_tx == htt->max_num_pending_tx - 1)
        ath10k_mac_tx_unlock(htt->ar, ATH10K_TX_PAUSE_Q_FULL);
}
...
static void ath10k_htt_tx_free_cont_txbuf(struct ath10k_htt *htt)
{
	...
	dma_free_coherent(htt->ar, htt->txbuf.memz);
...
static int ath10k_htt_tx_alloc_cont_txbuf(struct ath10k_htt *htt)
{
    ...
    htt->txbuf.memz = dma_zalloc_coherent(htt->ar, size, "htt_txbuf", 0, SOCKET_ID_ANY);
    htt->txbuf.vaddr = htt->txbuf.memz->addr;
    htt->txbuf.paddr = htt->txbuf.memz->phys_addr;
...
static void ath10k_htt_tx_free_cont_frag_desc(struct ath10k_htt *htt)
{
	...
	dma_free_coherent(htt->ar, htt->frag_desc.memz);
...
static int ath10k_htt_tx_alloc_cont_frag_desc(struct ath10k_htt *htt)
{
	...
	size = htt->max_num_pending_tx * sizeof(struct htt_msdu_ext_desc);
	htt->frag_desc.memz = dma_zalloc_coherent(htt->ar, size, "htt_tx_fg_ds", 0, 0);
	htt->frag_desc.vaddr = htt->frag_desc.memz->addr;
	htt->frag_desc.paddr = htt->frag_desc.memz->phys_addr;
	if (!htt->frag_desc.vaddr) {
...
static void ath10k_htt_tx_free_txq(struct ath10k_htt *htt)
{
	...
	if(htt->tx_q_state.memz != NULL) {
		dma_free_coherent(htt->ar, htt->tx_q_state.memz);
		htt->tx_q_state.memz = NULL;
		htt->tx_q_state.vaddr = NULL;
		htt->tx_q_state.paddr = NULL;
	}
}
...
static int ath10k_htt_tx_alloc_txq(struct ath10k_htt *htt)
{
	...
	htt->tx_q_state.memz = dma_zalloc_coherent(htt->ar, size, "htt_txqs", 0, 0);

	htt->tx_q_state.vaddr = htt->tx_q_state.memz->addr;
	if (!htt->tx_q_state.vaddr)
		return -ENOMEM;

	htt->tx_q_state.paddr = htt->tx_q_state.memz->phys_addr;;
...
static int ath10k_htt_tx_alloc_txdone_fifo(struct ath10k_htt *htt)
{
	...
	ret = rte_ring_init(&htt->txdone_fifo, "htt_txdf", size, SOCKET_ID_ANY);
	return ret;
}

int ath10k_htt_tx_alloc_buf(struct ath10k_htt *htt)
{
	struct ath10k *ar = htt->ar;
	int ret, size;
...
int ath10k_htt_tx_start(struct ath10k_htt *htt)
{
	...
	rte_spinlock_init(&htt->tx_lock);
	idr_init(&htt->pending_tx, htt->max_num_pending_tx);
...
int ath10k_htt_h2t_ver_req_msg(struct ath10k_htt *htt)
{
	...
	skb_put(skb, len);
	cmd = (struct htt_cmd *)skb_data(skb);
	cmd->hdr.msg_type = HTT_H2T_MSG_TYPE_VERSION_REQ;

	ath10k_dbg(ar, ATH10K_DBG_HTT, "Send version request on pipe: %i\n", htt->eid);
			     ...
			     size_t num_records)
{
	...
	rte_memcpy(cmd->tx_fetch_resp.records, records,
...
static u8 ath10k_htt_tx_get_tid(struct sk_buff *skb, bool is_eth)
{
	...
	if (!is_eth && ieee80211_is_mgmt(hdr->frame_control)) {
		...
		return 0 % IEEE80211_QOS_CTL_TID_MASK;
	} else
...
int ath10k_htt_mgmt_tx(struct ath10k_htt *htt, struct sk_buff *msdu)
{
	...
	int msdu_id = -1;
	int res;
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb_data(msdu);
	...
	skb_cb->paddr = skb_dma_addr(msdu);
	if (skb_cb->paddr == NULL) {
		res = -EIO;
		goto err_free_txdesc;
	}

	skb_put(txdesc, len);
	cmd = (struct htt_cmd *)skb_data(txdesc);
	memset(cmd, 0, len);

	cmd->hdr.msg_type         = HTT_H2T_MSG_TYPE_MGMT_TX;
	cmd->mgmt_tx.msdu_paddr = __cpu_to_le32(ATH10K_SKB_CB(msdu)->paddr);
	cmd->mgmt_tx.len        = __cpu_to_le32(skb_len(msdu));
	cmd->mgmt_tx.desc_id    = __cpu_to_le32(msdu_id);
	cmd->mgmt_tx.vdev_id    = __cpu_to_le32(vdev_id);
	rte_memcpy(cmd->mgmt_tx.hdr, skb_data(msdu),
	       min_t(int, skb_len(msdu), HTT_MGMT_FRM_HDR_DOWNLOAD_LEN));
		  ...
		  struct sk_buff *msdu, bool more)
{
	...
	prefetch_len = min_t(int, htt->prefetch_len, skb_len(msdu));
	prefetch_len = roundup(prefetch_len, 4);

	txbuf = &htt->txbuf.vaddr[msdu_id];
	txbuf_paddr = htt->txbuf.paddr + (sizeof(struct ath10k_htt_txbuf) * msdu_id);
	...
	if (unlikely(skb_cb->htt.is_offchan))
	...
	case ATH10K_HW_TXRX_ETHERNET:
		if (ar->hw_params.continuous_frag_desc) {
			...
			frags[0].tword_addr.len_16 = __cpu_to_le16(skb_len(msdu));
			...
			frags[0].dword_addr.paddr =
				__cpu_to_le32(skb_cb->paddr);
			frags[0].dword_addr.len = __cpu_to_le32(skb_len(msdu));
	...
	// if (msdu->ol_flags & PKT_TX_OFFLOAD_MASK &&
	...
	txbuf->cmd_tx.len = __cpu_to_le16(skb_len(msdu));
	...
	// 	   flags0, flags1, skb_len(msdu), msdu_id, frags_paddr,
	// 	   (u32)skb_cb->paddr, vdev_id, tid, freq);
	// ath10k_dbg_dump(ar, ATH10K_DBG_HTT_DUMP, NULL, "htt tx msdu: ",
	// 		skb_data(msdu), skb_len(msdu));
	...
	sg_items[1].vaddr = skb_data(msdu);
	sg_items[1].paddr = skb_cb->paddr;
	sg_items[1].len = prefetch_len;

	res = ath10k_hif_tx_sg(htt->ar,
			       htt->ar->htc.endpoint[htt->eid].ul_pipe_id,
			       sg_items, ARRAY_SIZE(sg_items), more);
	if (res)
		goto err_free_msdu_id;

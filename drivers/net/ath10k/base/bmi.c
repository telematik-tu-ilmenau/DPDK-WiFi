...
void ath10k_bmi_start(struct ath10k *ar)
{
	...
	if (ar->hw_params.hw_ops->enable_pll_clk != NULL) {
			       ...
			       struct bmi_target_info *target_info)
{
	...
	ath10k_dbg(ar, ATH10K_DBG_BMI, "version: 0x%x type: 0x%x\n", target_info->version, target_info->type);
			   ...
			   u32 address, void *buffer, u32 length)
{
	...
	while (length) {
		...
		rte_memcpy(buffer, resp.read_mem.payload, rxlen);
		address += rxlen;
		buffer  += rxlen;
		length  -= rxlen;
	}
			    ...
			    u32 address, const void *buffer, u32 length)
{
	...
	while (length) {
		txlen = min_t(u32, length, BMI_MAX_DATA_SIZE - hdrlen);

		/* copy before roundup to avoid reading beyond buffer*/
		rte_memcpy(cmd.write_mem.payload, buffer, txlen);
...
int ath10k_bmi_lz_data(struct ath10k *ar, const void *buffer, u32 length)
{
	...
	while (length) {
		...
		rte_memcpy(cmd.lz_data.payload, buffer, txlen);
			     ...
			     u32 address, const void *buffer, u32 length)
{
	...
	if (trailer_len > 0)
		rte_memcpy(trailer, buffer + head_len, trailer_len);

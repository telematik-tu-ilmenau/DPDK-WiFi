			  ...
			  unsigned int flags)
{
	...
	sdesc.addr   = rte_cpu_to_le_32(buffer);
	sdesc.nbytes = rte_cpu_to_le_16(nbytes);
	sdesc.flags  = rte_cpu_to_le_16(desc_flags);
	...
	*desc = sdesc;
	...
	/* WORKAROUND */
	if (!(flags & CE_SEND_NO_IDX_UPDATE))
...
int __ath10k_ce_rx_post_buf(struct ath10k_ce_pipe *pipe, void *ctx, u32 paddr)
{
	...
	desc->addr = rte_cpu_to_le_32(paddr);
	desc->nbytes = 0;
			 ...
			 const struct ce_attr *attr)
{
	...
	src_ring->base_addr_owner_space_unaligned = src_ring->memz->addr;
	base_addr = src_ring->memz->phys_addr;
			  ...
			  const struct ce_attr *attr)
{
	...
	dest_ring->base_addr_owner_space_unaligned = dest_ring->memz->addr;
	base_addr = dest_ring->memz->phys_addr;
...
void ath10k_ce_free_pipe(struct ath10k *ar, int ce_id)
{
	...
	if (ce_state->src_ring) {
		dma_free_coherent(ar, ce_state->src_ring->memz);
		kfree(ce_state->src_ring);
	}

	if (ce_state->dest_ring) {
		dma_free_coherent(ar, ce_state->dest_ring->memz);

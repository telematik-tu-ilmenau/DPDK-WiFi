...
u8 ath10k_mac_bitrate_to_rate(int bitrate)
{
...
u8 ath10k_parse_mpdudensity(u8 mpdudensity)
{
/*
...
int ath10k_peer_create(struct ath10k *ar,
...
// 	ether_addr_copy(sta->addr, arg.addr);
...
void ath10k_mac_vif_beacon_free(struct ath10k_vif *arvif)
{
 	...
 	if (!arvif->beacon_buf) {
 	    assert(false); // our firmware does not use this
 	...
 	ath10k_free_mbuf(ar, arvif->beacon);
...
int ath10k_recalc_rtscts_prot(struct ath10k_vif *arvif)
{
	struct ath10k *ar = arvif->ar;
...
void ath10k_control_ibss(struct ath10k_vif *arvif,
				struct ieee80211_bss_conf *info,
...
// static void ath10k_mac_vif_ap_csa_work(struct task *work)
// {
...
// 						   skb);
     ...
     if(!arvif) {
          RTE_LOG(ERR, PMD, "arvif list empty or invalid entry\n");
          return;
      }

     if(LIST_NEXT(arvif, pointers) != NULL) {
         RTE_LOG(ERR, PMD, "we do not expect to have more than one ath10k_vif\n");
         return;
     }

     LIST_FOREACH(peer, &ar->peers, pointers) {
         if(is_same_ether_addr(peer->addr, ieee80211_get_SA(hdr))) {
             return;
         }
     }

     ath10k_dbg(ar, ATH10K_DBG_WMI, "mac_addr %02x:%02x:%02x:%02x:%02x:%02x\n", ieee80211_get_SA(hdr)[0], ieee80211_get_SA(hdr)[1], ieee80211_get_SA(hdr)[2], ieee80211_get_SA(hdr)[3], ieee80211_get_SA(hdr)[4], ieee80211_get_SA(hdr)[5]);

     // ret = ath10k_peer_create(ar, arvif->vif, NULL, arvif->vdev_id, ieee80211_get_SA(hdr), WMI_PEER_TYPE_DEFAULT); // addr2 should be transmitter address in our case
     // if (ret) {
     ...
     rte_spinlock_lock(&ar->data_lock);
     peer = ath10k_peer_find(ar, arvif->vdev_id, hdr->addr2);
     rte_spinlock_unlock(&ar->data_lock);
     if (!peer) {
         ath10k_warn(ar, "failed to lookup peer %pM on vdev %i\n", hdr->addr2, arvif->vdev_id);
         return;
     }
    ...
    arvif->peer_id = find_first_bit(peer->peer_ids,
     ...
     // TODO we need to maintain ar->peer_map
}
...
// static void ath10k_mac_vif_sta_connection_loss_work(struct task *work)
// {
// 	struct ath10k_vif *arvif = container_of(work, struct ath10k_vif,
...
//      vht_mcs = __le16_to_cpu(vht_cap->vht_mcs.rx_mcs_map) >> (2 * i) & 3;
//
...
// 	ether_addr_copy(bss_conf->bssid, arvif->bssid);
...
// 	rte_spinlock_unlock(&ar->htt.tx_lock);
 ...
 ath10k_mac_tx_h_get_txmode(struct ath10k *ar,
 			   ...
 			   struct sk_buff *skb)
 {
 	const struct ieee80211_hdr *hdr = (void *)skb_data(skb);
 ...
 static void ath10k_tx_h_nwifi(struct ieee80211_hw *hw, struct sk_buff *skb)
 {
 	...
 	memmove(skb_data(skb) + IEEE80211_QOS_CTL_LEN,
 	       skb_data(skb), (u8*)qos_ctl - (u8*)skb_data(skb));
 ...
 static void ath10k_tx_h_8023(struct sk_buff *skb)
 {
 	...
 	rfc1042 = (void *)skb_data(skb) + hdrlen;
 	...
 	eth = (void *)skb_data(skb);
 	ether_addr_copy(da, &eth->d_addr.addr_bytes);
 	ether_addr_copy(sa, &eth->s_addr.addr_bytes);
 	eth->ether_type = type;
 }
 				       ...
 				       struct sk_buff *skb)
 {
 	...
 	if (unlikely(ieee80211_is_probe_resp(hdr->frame_control))) {
 		...
 		if (arvif->u.ap.noa_data) {
 		   ...
 		   char* newAddr = rte_pktmbuf_append(skb, arvif->u.ap.noa_len);
 		   if(newAddr) {
 				rte_memcpy(newAddr, arvif->u.ap.noa_data, arvif->u.ap.noa_len);
 		   }
 		}
 		rte_spinlock_unlock(&ar->data_lock);
 	}
...
bool ath10k_mac_tx_frm_has_freq(struct ath10k *ar)
{
 ...
 {
    skb_tailq_t *q = &ar->wmi_mgmt_tx_queue;
 	int ret = 0;
 	unsigned int len = 0;
 	...
 	if (get_skb_tailq_len(q) == ATH10K_MAX_NUM_MGMT_PENDING) {
 	...
 	TAILQ_INSERT_TAIL(q, skb, pointers_tailq);
 				...
 				bool more)
 {
 	...
 	case ATH10K_MAC_TX_HTT:
 		ret = ath10k_htt_tx(htt, txmode, skb, more);
 		break;
 			 ...
 			 bool more)
 {
 	...
 	// rte_hexdump(stdout, "mbuf right before tx helpers", rte_pktmbuf_mtod(skb, unsigned char*), rte_pktmbuf_data_len(skb));
 	...
 	if (info->flags & IEEE80211_TX_CTL_TX_OFFCHAN) {
 		if (!ath10k_mac_tx_frm_has_freq(ar)) {
 			...
 			TAILQ_INSERT_TAIL(&ar->offchan_tx_queue, skb, pointers_tailq);
 			ieee80211_queue_work(hw, &ar->offchan_tx_work);
 			return 0;
 		}
 	}

 	// rte_hexdump(stdout, "mbuf right before sending", rte_pktmbuf_mtod(skb, unsigned char*), rte_pktmbuf_data_len(skb));
 	ret = ath10k_mac_tx_submit(ar, txmode, txpath, skb, more);
 	if (ret) {
...
void ath10k_offchan_tx_work(struct task *work)
{
...
void ath10k_mgmt_over_wmi_tx_work(struct task *work)
{
...
// }
 ...
 bool ath10k_mac_tx_can_push_ath10k_args(struct ath10k *ar,
                   struct ath10k_txq *artxq)
 {
    //  /* No need to get locks */
    //  assert(ar->htt.tx_q_state.mode == HTT_TX_MODE_SWITCH_PUSH || ar->htt.tx_q_state.mode == HTT_TX_MODE_SWITCH_PUSH_PULL);
    //  // RTE_LOG(DEBUG, PMD, "mode: %s, num_pending_tx: %d < %d, num_fw_queued: %d < %d\n", ar->htt.tx_q_state.mode == HTT_TX_MODE_SWITCH_PUSH ? "PUSH" : "PULL", ar->htt.num_pending_tx, ar->htt.tx_q_state.num_push_allowed, artxq->num_fw_queued, artxq->num_push_allowed);
 ...
 bool ath10k_mac_tx_can_push(struct ieee80211_hw *hw,
 				   struct ieee80211_txq *txq)
 {
 	struct ath10k *ar = hw->priv;
 			   ...
 			   struct ieee80211_txq *txq)
 {
    ...
    rte_spinlock_lock(&ar->mpool_lock);
 	skb = ieee80211_tx_dequeue(hw, txq, ar->mpool);
...
void ath10k_mac_tx_push_pending(struct ath10k *ar)
{
	...
	last = TAILQ_LAST(&ar->txqs, ath10k_txq_tailq);
 	// while (!list_empty(&ar->txqs)) {
	while (!TAILQ_EMPTY(&ar->txqs)) {
 		// artxq = list_first_entry(&ar->txqs, struct ath10k_txq, list);
	    artxq = TAILQ_FIRST(&ar->txqs);
 		...
 		// list_del_init(&artxq->list);
 		TAILQ_REMOVE(&ar->txqs, artxq, pointers);
 		if (ret != -ENOENT)
...
// void ath10k_scan_timeout_work(struct task *work)
// {
...
void ath10k_mac_op_tx(struct ieee80211_hw *hw,
             ...
             struct sk_buff *skb, bool more)
{
    ...
    ret = ath10k_mac_tx(ar, vif, txmode, txpath, skb, more);
    if (ret) {
...
int ath10k_start(struct ieee80211_hw *hw)
{
	struct ath10k *ar = hw->priv;
	u32 param;
	int ret = 0;
...
// 	task_create(&arvif->ap_csa_work, (task_fn_t *) ath10k_mac_vif_ap_csa_work);
...
// 		rte_memcpy(arg.ie, req->ie, arg.ie_len);
// 	}
...
// 	rte_spinlock_lock(&arvif->ar->conf_lock);
...
// unlock:
// 	rte_spinlock_unlock(&arvif->ar->conf_lock);
// }

// static void ath10k_sta_rc_update_wk(struct task *wk)
// {
// 	struct ath10k *ar;
// 	struct ath10k_vif *arvif;
...
// 		arsta->arvif = arvif;
// 		task_create(&arsta->update_wk, (task_fn_t *) ath10k_sta_rc_update_wk);
...
// 	rte_spinlock_lock(&ar->data_lock);
// 	rte_memcpy(survey, ar_survey, sizeof(*survey));
// 	rte_spinlock_unlock(&ar->data_lock);
...
// void ath10k_offset_tsf(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
// 						   int64_t tsf_offset)
// {
/*
	struct ath10k *ar = hw->priv;
...
struct ath10k *ath10k_mac_create(size_t priv_size)
{
	...
	struct ath10k *ar;
    ar = rte_zmalloc(NULL, sizeof(struct ath10k) + priv_size, 0);
    if (!ar)
	    return NULL;
    return ar;
}
...
// 		ar->ops->wake_tx_queue = NULL;
    ...
    ret = ath_regd_init(&ar->ath_common.regulatory, ar->hw->wiphy, NULL /*ath10k_reg_notifier*/);
    if (ret) {

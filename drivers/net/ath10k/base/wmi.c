...
static struct wmi_vdev_param_map wmi_vdev_param_map = {
	...
	.inc_tsf = WMI_VDEV_PARAM_UNSUPPORTED,
	.dec_tsf = WMI_VDEV_PARAM_UNSUPPORTED
};
...
static struct wmi_vdev_param_map wmi_10_2_4_vdev_param_map = {
	...
	.rx_decap_type = WMI_VDEV_PARAM_UNSUPPORTED,
	.bw_nss_ratemask = WMI_VDEV_PARAM_UNSUPPORTED,
	.inc_tsf = WMI_10X_VDEV_PARAM_TSF_INCREMENT,
};
...
struct sk_buff *ath10k_wmi_alloc_skb(struct ath10k *ar, u32 len)
{
	...
	// printf("wmi_alloc_skb (1) %i\n", skb_len(skb));
	// ath10k_dbg(ar, ATH10K_DBG_HTC, "wmi alloc skb: %p\n", skb);

	skb_reserve(skb, WMI_SKB_HEADROOM);
	if (!IS_ALIGNED((unsigned long)skb_data(skb), 4))
		ath10k_warn(ar, "Unaligned WMI skb\n");
	// printf("wmi_alloc_skb (2) %i\n", skb_len(skb));

	skb_put(skb, round_len);
	memset(skb_data(skb), 0, round_len);
	// printf("wmi_alloc_skb (3) %i\n", skb_len(skb));
			       ...
			       u32 cmd_id)
{
	...
	// printf("cmd_send_nowait (1) %i\n", skb_len(skb));
	if (skb_push(skb, sizeof(struct wmi_cmd_hdr)) == NULL) {
		ath10k_dbg(ar, ATH10K_DBG_WMI, "Error on skb_push in wmi_cmd_send_nowait?!\n");
		return -ENOMEM;
	}
	// printf("cmd_send_nowait (2) %i\n", skb_len(skb));

	cmd |= SM(cmd_id, WMI_CMD_HDR_CMD_ID);

	cmd_hdr = (struct wmi_cmd_hdr *)skb_data(skb);
	cmd_hdr->cmd_id = __cpu_to_le32(cmd);

	// RTE_LOG(ERR, PMD, "BEGIN ath10k_wmi_cmd_send_nowait\n");
	// RTE_LOG(ERR, PMD, "sending wmi command with id %d %d (%s)\n", cmd_id, cmd, wmi_cmd_id_name(cmd_id));

 //    RTE_LOG(ERR, PMD, "skb->data: %p, skb->len: %d\n", skb_data(skb), skb_len(skb));
 //    if (skb_len(skb) > 0) {
 //        rte_hexdump(stdout, "wmi_cmd_skb (ath10k_wmi_cmd_send_nowait): ", skb_data(skb), skb_len(skb));
 //    }

 //    RTE_LOG(ERR, PMD, "END ath10k_wmi_cmd_send_nowait\n");

	memset(skb_cb, 0, sizeof(*skb_cb));
	// printf("cmd_send_nowait (3) %i\n", skb_len(skb));
	ret = ath10k_htc_send(&ar->htc, ar->wmi.eid, skb);
	// trace_ath10k_wmi_cmd(ar, cmd_id, skb_data(skb), skb_len(skb), ret);
...
static void ath10k_wmi_tx_beacon_nowait(struct ath10k_vif *arvif)
{
    ...
    if (!bcn) {
        RTE_LOG(DEBUG, PMD, "beacon is not set within ath10k_vif\n");
        goto unlock;
    }
...
static void ath10k_wmi_op_ep_tx_credits(struct ath10k *ar)
{
	...
	ath10k_dbg(ar, ATH10K_DBG_WMI, "Got WMI tx credit\n");

	completion_done(&ar->wmi.tx_credits_compl);
...
int ath10k_wmi_cmd_send(struct ath10k *ar, struct sk_buff *skb, u32 cmd_id)
{
	...
	uint64_t to = rte_get_timer_cycles() + rte_get_timer_hz() * 10;
    do {
		reinit_completion(&ar->wmi.tx_credits_compl);
		...
		if(ret) {
			wait_for_completion_timeout(&ar->wmi.tx_credits_compl, 3000);
		}
    } while(ret && to > rte_get_timer_cycles());
...
ath10k_wmi_op_gen_mgmt_tx(struct ath10k *ar, struct sk_buff *msdu)
{
	...
	int len;
	u32 vdev_id;
	u32 buf_len = skb_len(msdu);
	u16 fc;

	hdr = (struct ieee80211_hdr *)skb_data(msdu);
	...
	len = sizeof(cmd->hdr) + skb_len(msdu);
	...
	// strcpy(skb->debug_name, "wmi mgmt tx");

	cmd = (struct wmi_mgmt_tx_cmd *)skb_data(skb);
	...
	ether_addr_copy((struct ether_addr *)ieee80211_get_DA(hdr), (struct ether_addr *)cmd->hdr.peer_macaddr.addr);
	rte_memcpy(cmd->buf, skb_data(msdu), skb_len(msdu));

	ath10k_dbg(ar, ATH10K_DBG_WMI, "wmi mgmt tx skb %pK len %d ftype %02x stype %02x\n",
		   msdu, skb_len(skb), fc & IEEE80211_FCTL_FTYPE,
		   fc & IEEE80211_FCTL_STYPE);
	// trace_ath10k_tx_hdr(ar, skb_data(skb), skb_len(skb));
	// trace_ath10k_tx_payload(ar, skb_data(skb), skb_len(skb));
...
{
	struct wmi_scan_event *ev = (void *)skb_data(skb);

	if (skb_len(skb) < sizeof(*ev))
		return -EPROTO;
					 ...
					 struct ieee80211_rx_status *status)
{
	...
	// if (skb_len(skb) < (hdrlen + IEEE80211_WEP_IV_LEN))
	// 	return;

	// keyidx = skb_data(skb)[hdrlen + (IEEE80211_WEP_IV_LEN - 1)] >> WEP_KEYID_SHIFT;
					 ...
					 struct wmi_mgmt_rx_ev_arg *arg)
{
	...
	if (test_bit(ATH10K_FW_FEATURE_EXT_WMI_MGMT_RX,
		...
		ev_v2 = (struct wmi_mgmt_rx_event_v2 *)skb_data(skb);
		ev_hdr = &ev_v2->hdr.v1;
		pull_len = sizeof(*ev_v2);
	} else {
		ev_v1 = (struct wmi_mgmt_rx_event_v1 *)skb_data(skb);
		ev_hdr = &ev_v1->hdr;
	...
	if (skb_len(skb) < msdu_len)
		return -EPROTO;

	if (le32_to_cpu(arg->status) & WMI_RX_STATUS_EXT_INFO) {
		len = ALIGN(le32_to_cpu(arg->buf_len), 4);
		ext_info = (struct wmi_mgmt_rx_ext_info *)(skb_data(skb) + len);
		rte_memcpy(&arg->ext_info, ext_info,
					      ...
					      struct wmi_mgmt_rx_ev_arg *arg)
{
	...
	ev = (struct wmi_10_4_mgmt_rx_event *)skb_data(skb);
	ev_hdr = &ev->hdr;
	pull_len = sizeof(*ev);
				       ...
				       struct ieee80211_hdr *hdr)
{
	 ...
	 */
	if (is_multicast_ether_addr((const struct ether_addr *)ieee80211_get_DA(hdr)) &&
...
int ath10k_wmi_event_mgmt_rx(struct ath10k *ar, struct sk_buff *skb)
{
	...
	ath10k_dbg(ar, ATH10K_DBG_MGMT,
		   "event mgmt rx status %08x len %u\n", rx_status, skb_len(skb));
	...
	if (ieee80211_is_beacon(hdr->frame_control)) {
		// ath10k_mac_handle_beacon(ar, skb); // obsolete .. this is being done in wifi_rx_mgmt
	}
	...
	// skb belongs to ieee80211 mgmt stack now
	rte_wifi_mgmt_rx(ar->dev->data->port_id, skb, status->mactime);
	return 0;
}
...
{
	struct wmi_chan_info_event *ev = (void *)skb_data(skb);
...
static int ath10k_wmi_10_4_op_pull_ch_info_ev(struct ath10k *ar,
					      ...
					      struct wmi_ch_info_ev_arg *arg)
{
	struct wmi_10_4_chan_info_event *ev = (void *)skb_data(skb);
...
int ath10k_wmi_event_debug_mesg(struct ath10k *ar, struct sk_buff *skb)
{
	...
	// rte_hexdump(stdout, "WMI DBGLOG MSG", skb_data(skb), skb_len(skb));
	// trace_ath10k_wmi_dbglog(ar, skb_data(skb), skb_len(skb));
				     ...
				     struct ath10k_fw_stats_pdev *dst)
{
	...
	printf("Noise floor %u\n", dst->ch_noise_floor);
	dst->tx_frame_count = __le32_to_cpu(src->tx_frame_count);
	printf("tx frame count %u\n", dst->tx_frame_count);
	dst->rx_frame_count = __le32_to_cpu(src->rx_frame_count);
	printf("rx frame count %u\n", dst->rx_frame_count);
	dst->rx_clear_count = __le32_to_cpu(src->rx_clear_count);
	printf("rx clear count %u\n", dst->rx_clear_count);
	dst->cycle_count = __le32_to_cpu(src->cycle_count);
	printf("cycle count %u\n", dst->cycle_count);
	dst->phy_err_count = __le32_to_cpu(src->phy_err_count);
	printf("phy err count %u\n", dst->phy_err_count);
	dst->chan_tx_power = __le32_to_cpu(src->chan_tx_pwr);
	printf("chan tx power %u\n", dst->chan_tx_power);
}
...
{
	dst->comp_queued = __le32_to_cpu(src->comp_queued);
	printf("comp_queued %u\n", dst->comp_queued);
	dst->comp_delivered = __le32_to_cpu(src->comp_delivered);
	printf("comp_delivered %u\n", dst->comp_delivered);
	dst->msdu_enqued = __le32_to_cpu(src->msdu_enqued);
	printf("msdu enqued %u\n", dst->msdu_enqued);
	dst->mpdu_enqued = __le32_to_cpu(src->mpdu_enqued);
	printf("mpdu enqued %u\n", dst->mpdu_enqued);
	dst->wmm_drop = __le32_to_cpu(src->wmm_drop);
	printf("wmm drop %u\n", dst->wmm_drop);
	dst->local_enqued = __le32_to_cpu(src->local_enqued);
	printf("local_enqued %u\n", dst->local_enqued);
	dst->local_freed = __le32_to_cpu(src->local_freed);
	printf("local freed %u\n", dst->local_freed);
	dst->hw_queued = __le32_to_cpu(src->hw_queued);
	printf("hw_queued %u\n", dst->hw_queued);
	dst->hw_reaped = __le32_to_cpu(src->hw_reaped);
	printf("hw_reaped %u\n", dst->hw_reaped);
	...
	printf("tx_abort %u\n", dst->tx_abort);
	dst->mpdus_requed = __le32_to_cpu(src->mpdus_requed);
	printf("mpdus requed %u\n", dst->mpdus_requed);
	dst->tx_ko = __le32_to_cpu(src->tx_ko);
	printf("tx_ko %u\n", dst->tx_ko);
	dst->data_rc = __le32_to_cpu(src->data_rc);
	printf("data rc %u\n", dst->data_rc);
	dst->self_triggers = __le32_to_cpu(src->self_triggers);
	printf("self_triggers %u\n", dst->self_triggers);
	dst->sw_retry_failure = __le32_to_cpu(src->sw_retry_failure);
	printf("sw_retry_failure %u\n", dst->sw_retry_failure);
	dst->illgl_rate_phy_err = __le32_to_cpu(src->illgl_rate_phy_err);
	printf("illgl_rate_phy_err %u\n", dst->illgl_rate_phy_err);
	dst->pdev_cont_xretry = __le32_to_cpu(src->pdev_cont_xretry);
	printf("pdev_cont_xretry %u\n", dst->pdev_cont_xretry);
	dst->pdev_tx_timeout = __le32_to_cpu(src->pdev_tx_timeout);
	printf("pdev_tx_timeout %u\n", dst->pdev_tx_timeout);
	dst->pdev_resets = __le32_to_cpu(src->pdev_resets);
	printf("pdev_resets %u\n", dst->pdev_resets);
	dst->phy_underrun = __le32_to_cpu(src->phy_underrun);
	printf("phy_underrun %u\n", dst->phy_underrun);
	dst->txop_ovf = __le32_to_cpu(src->txop_ovf);
	printf("txop_ovf %u\n", dst->txop_ovf);
				      ...
				      struct ath10k_fw_stats_pdev *dst)
{
	...
	printf("mib_int_count %u\n", dst->mib_int_count);
}

void ath10k_wmi_pull_peer_stats(const struct wmi_peer_stats *src,
				struct ath10k_fw_stats_peer *dst)
{
	ether_addr_copy((struct ether_addr *)src->peer_macaddr.addr, (struct ether_addr *)dst->peer_macaddr);
...
{
	const struct wmi_stats_event *ev = (void *)skb_data(skb);
...
static int ath10k_wmi_10_2_op_pull_fw_stats(struct ath10k *ar,
					    ...
					    struct ath10k_fw_stats *stats)
{
	const struct wmi_10_2_stats_event *ev = (void *)skb_data(skb);
	u32 num_pdev_stats;
...
void print_stats(struct ath10k *ar, struct sk_buff *skb) {
	printf("STATS\n");
	rte_spinlock_lock(&ar->data_lock);

	struct ath10k_fw_stats stats = {};

	int ret = ath10k_wmi_pull_fw_stats(ar, skb, &stats);
	if (ret) {
...
{
	struct wmi_vdev_start_response_event *ev = (void *)skb_data(skb);
...
{
	struct wmi_peer_sta_kickout_event *ev = (void *)skb_data(skb);
...
{
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb_data(bcn);
	...
	if (pvm_len < arvif->u.ap.tim_len) {
		...
		int move_size = (int) skb_tail_pointer(bcn) - (int) (ie + 2 + ie_len);
	...
	rte_memcpy(tim->virtual_map, arvif->u.ap.tim_bitmap, pvm_len);
...
{
	struct wmi_host_swba_event *ev = (void *)skb_data(skb);
	u32 map;
	size_t i;
...
static int ath10k_wmi_10_2_4_op_pull_swba_ev(struct ath10k *ar,
					     ...
					     struct wmi_swba_ev_arg *arg)
{
	struct wmi_10_2_4_host_swba_event *ev = (void *)skb_data(skb);
	u32 map;
	size_t i;
...
static int ath10k_wmi_10_4_op_pull_swba_ev(struct ath10k *ar,
					   ...
					   struct wmi_swba_ev_arg *arg)
{
	struct wmi_10_4_host_swba_event *ev = (void *)skb_data(skb);
	u32 map, tim_len;
	size_t i;
...
{
	// copied from linux mon0
	static const char beacon_template[] = 
	"\x80\x00\x00\x00\xff\xff\xff\xff\xff\xff\x04\xf0\x21\x30\x47\x15" \
	"\x04\xf0\x21\x30\x47\x15\x50\xd2" \
	"\x01\x80\xf1\xd6\x43\x00\x00\x00\xe8\x03\x00\x00\x00\x00\x01\x08" \
	"\x8c\x12\x98\x24\xb0\x48\x60\x6c\x03\x01\x24\x05\x04\x01\x02\x00" \
	"\x00\x2d\x1a\xef\x19\x1b\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00" \
	"\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x3d\x16\x24" \
	"\x00\x00\x00\x00\x00\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00\x00" \
	"\x00\x00\x00\x00\x00\x72\x08\x4e\x46\x56\x5f\x4d\x45\x53\x48\x71" \
	"\x07\x01\x01\x00\x01\x00\x00\x09\xbf\x0c\xfa\x79\x99\x33\xfa\xff" \
	"\x0c\x03\xfa\xff\x0c\x03\xc0\x05\x00\x24\x00\xff\xff";
	...
	for (; map; map >>= 1, vdev_id++) {
		 ...
		 */
		if (!arvif->is_up) {
			printf("SWBA: continue, because arvif is down\n");
			continue;
		}
		...
		if (arvif->vif->csa_active &&
			...
			printf("SWBA: continue, because csa is active and complete\n");
			ieee80211_csa_finish(arvif->vif);
			continue;
		}

  //       // rte_spinlock_lock(&ar->mpool_lock);
		// bcn = ieee80211_beacon_get(ar->mpool, ar->hw, arvif->vif);
  //       // rte_spinlock_unlock(&ar->mpool_lock);
            bcn = ath10k_alloc_mbuf(ar, sizeof(beacon_template));
            rte_pktmbuf_append(bcn, sizeof(beacon_template) - 1);
            rte_memcpy(skb_data(bcn), beacon_template, sizeof(beacon_template) - 1);

            struct ieee80211_mgmt* beacon_mgmt = skb_data(bcn);
            rte_memcpy(&beacon_mgmt->sa, arvif->bssid, 6);
            rte_memcpy(&beacon_mgmt->bssid, arvif->bssid, 6);
		...
		if (!arvif->beacon_buf) {
			paddr = rte_mbuf_data_dma_addr(bcn);
			ret = paddr == NULL;
			if (ret) {
			...
			if (skb_len(bcn) > IEEE80211_MAX_FRAME_LEN) {
				ath10k_warn(ar, "trimming beacon %d -> %d bytes!\n",
					    skb_len(bcn), IEEE80211_MAX_FRAME_LEN);
				skb_trim(bcn, IEEE80211_MAX_FRAME_LEN);
			}
			rte_memcpy(arvif->beacon_buf, skb_data(bcn), skb_len(bcn));
...
{
	struct wmi_phyerr_event *ev = (void *)skb_data(skb);
	...
	arg->buf_len = skb_len(skb) - sizeof(*ev);
...
static int ath10k_wmi_10_4_op_pull_phyerr_ev_hdr(struct ath10k *ar,
						 ...
						 struct wmi_phyerr_hdr_arg *arg)
{
	struct wmi_10_4_phyerr_event *ev = (void *)skb_data(skb);
	...
	arg->tsf_l32 = __le32_to_cpu(ev->tsf_l32);
	arg->tsf_u32 = __le32_to_cpu(ev->tsf_u32);
	arg->buf_len = skb_len(skb);
	arg->phyerrs = skb_data(skb);
					  ...
					  u8 *rate_code, u16 *pream_table, u8 type)
{
	...
	for (i = 0; i < __le32_to_cpu(ev->rate_max); i++) {
		...
		rte_memcpy(tpc_stats->tpc_table[type].tpc_value[i],
...
void ath10k_wmi_event_pdev_tpc_config(struct ath10k *ar, struct sk_buff *skb)
{
	...
	ev = (struct wmi_pdev_tpc_config_event *)skb_data(skb);
	...
	// ath10k_dbg(ar, ATH10K_DBG_WMI,
	printf("wmi event tpc config channel %d mode %d ctl %d regd %d gain %d %d limit %d max_power %d tx_chains %d rates %d\n",
				  ...
				  u32 num_units, u32 unit_len)
{
	...
	struct rte_memzone* memz = dma_zalloc_coherent(ar, pool_size, "WMI_CHUNK", idx, SOCKET_ID_ANY);

	if (memz == NULL)
		return -ENOMEM;

	ar->wmi.chunk_memzones[idx] = memz;
	ar->wmi.mem_chunks[idx].vaddr = memz->addr;
	ar->wmi.mem_chunks[idx].paddr = memz->phys_addr;
...
static void ath10k_wmi_event_service_ready_work(struct task *work)
{
	struct ath10k *ar = container_of(work, struct ath10k, svc_rdy_work);
	struct sk_buff *skb = ar->svc_rdy_skb;
	struct wmi_svc_rdy_ev_arg arg = {};
	u32 num_units, req_id, unit_size, num_mem_reqs, num_unit_info, i, j;
	int ret;
	bool allocated;
	...
	ath10k_dbg(ar, ATH10K_DBG_WMI, "wmi svc\n");
	...
	for(i = 0; i < num_mem_reqs; ++i) {
		for(j = 0; j < num_mem_reqs; ++j) {
			if(i != j) {
				assert(ar->wmi.mem_chunks[i].vaddr != ar->wmi.mem_chunks[j].vaddr);
			}
		}
	}
	assert(ar->wmi.num_mem_chunks == num_mem_reqs);
...
{
	ar->svc_rdy_skb = skb;
	taskqueue_enqueue_sync(ar->workqueue_aux, &ar->svc_rdy_work);
}

static int ath10k_wmi_op_pull_rdy_ev(struct ath10k *ar, struct sk_buff *skb,
				     struct wmi_rdy_ev_arg *arg)
{
	struct wmi_ready_event *ev = (void *)skb_data(skb);
...
static int ath10k_wmi_op_pull_roam_ev(struct ath10k *ar, struct sk_buff *skb,
				      struct wmi_roam_ev_arg *arg)
{
	struct wmi_roam_ev *ev = (void *)skb_data(skb);
...
static int ath10k_wmi_op_pull_echo_ev(struct ath10k *ar,
				      ...
				      struct wmi_echo_ev_arg *arg)
{
	struct wmi_echo_event *ev = (void *)skb_data(skb);
...
int ath10k_wmi_event_ready(struct ath10k *ar, struct sk_buff *skb)
{
	...
	ath10k_dbg(ar, ATH10K_DBG_WMI,
		   "wmi event ready sw_version %u abi_version %u mac_addr %02x:%02x:%02x:%02x:%02x:%02x status %d\n",
	...
	ether_addr_copy((struct ether_addr *)arg.mac_addr, (struct ether_addr *)ar->mac_addr);

	ath10k_dbg(ar, ATH10K_DBG_WMI,
		   "ar mac_addr %02x:%02x:%02x:%02x:%02x:%02x\n",
		   ar->mac_addr[0],
		   ar->mac_addr[1],
		   ar->mac_addr[2],
		   ar->mac_addr[3],
...
static int ath10k_wmi_event_temperature(struct ath10k *ar, struct sk_buff *skb)
{
	...
	ev = (struct wmi_pdev_temperature_event *)skb_data(skb);
	if (WARN_ON(skb_len(skb) < sizeof(*ev)))
		return -EPROTO;

	printf("PDEV temp: %i\n", __le32_to_cpu(ev->temperature));
					       ...
					       struct sk_buff *skb)
{
	...
	ev = (struct wmi_pdev_bss_chan_info_event *)skb_data(skb);
	if (WARN_ON(skb_len(skb) < sizeof(*ev)))
		return -EPROTO;
...
static void ath10k_wmi_op_rx(struct ath10k *ar, struct sk_buff *skb)
{
	...
	// trace_ath10k_wmi_event(ar, id, skb_data(skb), skb_len(skb));
...
static void ath10k_wmi_10_4_op_rx(struct ath10k *ar, struct sk_buff *skb)
{
	...
	case WMI_10_4_MGMT_RX_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_MGMT_RX_EVENTID\n");
		ath10k_wmi_event_mgmt_rx(ar, skb);
		/* mgmt_rx() owns the skb now! */
		return;
	case WMI_10_4_ECHO_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_ECHO_EVENTID\n");
		ath10k_wmi_event_echo(ar, skb);
		break;
	case WMI_10_4_DEBUG_MESG_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_DEBUG_MESG_EVENTID\n");
		ath10k_wmi_event_debug_mesg(ar, skb);
		ath10k_wmi_queue_set_coverage_class_work(ar);
		break;
	case WMI_10_4_SERVICE_READY_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_SERVICE_READY_EVENTID\n");
		ath10k_wmi_event_service_ready(ar, skb);
		return;
	case WMI_10_4_SCAN_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_SCAN_EVENTID\n");
		ath10k_wmi_event_scan(ar, skb);
		ath10k_wmi_queue_set_coverage_class_work(ar);
		break;
	case WMI_10_4_CHAN_INFO_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_CHAN_INFO_EVENTID\n");
		ath10k_wmi_event_chan_info(ar, skb);
		break;
	case WMI_10_4_PHYERR_EVENTID:
	    ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_PHYERR_EVENTID\n");
		ath10k_wmi_event_phyerr(ar, skb);
		break;
	case WMI_10_4_READY_EVENTID:
	    ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_READY_EVENTID\n");
		ath10k_wmi_event_ready(ar, skb);
		ath10k_wmi_queue_set_coverage_class_work(ar);
		break;
	case WMI_10_4_PEER_STA_KICKOUT_EVENTID:
		ath10k_wmi_event_peer_sta_kickout(ar, skb);
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_PEER_STA_KICKOUT_EVENTID\n");
		break;
	case WMI_10_4_ROAM_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_ROAM_EVENTID\n");
		ath10k_wmi_event_roam(ar, skb);
		ath10k_wmi_queue_set_coverage_class_work(ar);
		break;
	case WMI_10_4_HOST_SWBA_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_HOST_SWBA_EVENTID\n");
		ath10k_wmi_event_host_swba(ar, skb);
		break;
	case WMI_10_4_TBTTOFFSET_UPDATE_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_TBTTOFFSET_UPDATE_EVENTID\n");
		ath10k_wmi_event_tbttoffset_update(ar, skb);
		break;
	case WMI_10_4_DEBUG_PRINT_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_DEBUG_PRINT_EVENTID\n");
		ath10k_wmi_event_debug_print(ar, skb);
		ath10k_wmi_queue_set_coverage_class_work(ar);
		break;
	case WMI_10_4_VDEV_START_RESP_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_VDEV_START_RESP_EVENTID\n");
		ath10k_wmi_event_vdev_start_resp(ar, skb);
		ath10k_wmi_queue_set_coverage_class_work(ar);
		break;
	case WMI_10_4_VDEV_STOPPED_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_VDEV_STOPPED_EVENTID\n");
		ath10k_wmi_event_vdev_stopped(ar, skb);
		ath10k_wmi_queue_set_coverage_class_work(ar);
		break;
	...
	case WMI_10_4_UPDATE_STATS_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_UPDATE_STATS_EVENTID\n");
		ath10k_wmi_event_update_stats(ar, skb);
		break;
	case WMI_10_4_PDEV_TEMPERATURE_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_PDEV_TEMPERATURE_EVENTID\n");
		ath10k_wmi_event_temperature(ar, skb);
		break;
	case WMI_10_4_PDEV_BSS_CHAN_INFO_EVENTID:
		ath10k_dbg(ar, ATH10K_DBG_WMI, "WMI_10_4_PDEV_BSS_CHAN_INFO_EVENTID\n");
		ath10k_wmi_event_pdev_bss_chan_info(ar, skb);
		break;
    case WMI_10_4_PDEV_TPC_CONFIG_EVENTID:
        ath10k_wmi_event_pdev_tpc_config(ar, skb);
        break;
   	case WMI_10_4_PDEV_GET_AST_INFO_EVENTID:
   		// ath10k_err(ar, "WMI_10_4_PDEV_GET_AST_INFO_EVENTID\n");
   		break;
    case WMI_10_4_MU_REPORT_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_MU_REPORT_EVENTID\n");
    	break;
    case WMI_10_4_TX_DATA_TRAFFIC_CTRL_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_TX_DATA_TRAFFIC_CTRL_EVENTID\n");
    	break;
    case WMI_10_4_PEER_TX_MU_TXMIT_COUNT_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_PEER_TX_MU_TXMIT_COUNT_EVENTID\n");
    	break;
    case WMI_10_4_PEER_GID_USERPOS_LIST_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_PEER_GID_USERPOS_LIST_EVENTID\n");
    	break;
    case WMI_10_4_PDEV_CHECK_CAL_VERSION_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_PDEV_CHECK_CAL_VERSION_EVENTID\n");
    	break;
    case WMI_10_4_ATF_PEER_STATS_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_ATF_PEER_STATS_EVENTID\n");
    	break;
    case WMI_10_4_PDEV_GET_RX_FILTER_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_PDEV_GET_RX_FILTER_EVENTID\n");
    	break;
    case WMI_10_4_NAC_RSSI_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_NAC_RSSI_EVENTID\n");
    	break;
    case WMI_10_4_DEBUG_FATAL_CONDITION_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_DEBUG_FATAL_CONDITION_EVENTID\n");
    	break;
    case WMI_10_4_GET_TSF_TIMER_RESP_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_GET_TSF_TIMER_RESP_EVENTID\n");
    	break;
    case WMI_10_4_PDEV_TPC_TABLE_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_PDEV_TPC_TABLE_EVENTID\n");
    	break;
    case WMI_10_4_PDEV_WDS_ENTRY_LIST_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_PDEV_WDS_ENTRY_LIST_EVENTID\n");
    	break;
    case WMI_10_4_TDLS_PEER_EVENTID:
    	// ath10k_err(ar, "WMI_10_4_TDLS_PEER_EVENTID\n");
    	break;
	default:
		ath10k_warn(ar, "Unknown eventid: %d\n", id);
		break;
			      ...
			      enum wmi_dfs_region dfs_reg)
{
	...
	cmd = (struct wmi_pdev_set_regdomain_cmd *)skb_data(skb);
				  ...
				  enum wmi_dfs_region dfs_reg)
{
	...
	cmd = (struct wmi_pdev_set_regdomain_cmd_10x *)skb_data(skb);
	cmd->reg_domain = __cpu_to_le32(rd);
	cmd->reg_domain_2G = __cpu_to_le32(rd2g);
	cmd->reg_domain_5G = __cpu_to_le32(rd5g);
	cmd->conformance_test_limit_2G = __cpu_to_le32(ctl2g);
...
ath10k_wmi_op_gen_pdev_suspend(struct ath10k *ar, u32 suspend_opt)
{
	...
	if (!skb)
		return ERR_PTR(-ENOMEM);
	// strcpy(skb->debug_name, "wmi pdev_suspend");

	cmd = (struct wmi_pdev_suspend_cmd *)skb_data(skb);
...
ath10k_wmi_op_gen_pdev_resume(struct ath10k *ar)
{
	...
	if (!skb)
		return ERR_PTR(-ENOMEM);
	// strcpy(skb->debug_name, "wmi pdev_resume");
...
ath10k_wmi_op_gen_pdev_set_param(struct ath10k *ar, u32 id, u32 value)
{
	...
	if (!skb)
		return ERR_PTR(-ENOMEM);
	// strcpy(skb->debug_name, "wmi pdev_set param");

	cmd = (struct wmi_pdev_set_param_cmd *)skb_data(skb);
...
static struct sk_buff *ath10k_wmi_op_gen_init(struct ath10k *ar)
{
	...
	cmd = (struct wmi_init_cmd *)skb_data(buf);

	rte_memcpy(&cmd->resource_config, &config, sizeof(config));
...
static struct sk_buff *ath10k_wmi_10_1_op_gen_init(struct ath10k *ar)
{
	...
	cmd = (struct wmi_init_cmd_10x *)skb_data(buf);
...
static struct sk_buff *ath10k_wmi_10_2_op_gen_init(struct ath10k *ar)
{
	...
	cmd = (struct wmi_init_cmd_10_2 *)skb_data(buf);
	...
	rte_memcpy(&cmd->resource_config.common, &config, sizeof(config));
	ath10k_wmi_put_host_mem_chunks(ar, &cmd->mem_chunks);
...
static struct sk_buff *ath10k_wmi_10_4_op_gen_init(struct ath10k *ar)
{
	...
	cmd = (struct wmi_init_cmd_10_4 *)skb_data(buf);
	rte_memcpy(&cmd->resource_config, &config, sizeof(config));
	ath10k_wmi_put_host_mem_chunks(ar, &cmd->mem_chunks);
			       ...
			       const struct wmi_start_scan_arg *arg)
{
	...
	if (arg->n_bssids) {
		...
		for (i = 0; i < arg->n_bssids; i++)
			ether_addr_copy((struct ether_addr *)arg->bssids[i].bssid, (struct ether_addr *)bssids->bssid_list[i].addr);
	...
	if (arg->ie_len) {
		...
		rte_memcpy(ie->ie_data, arg->ie, arg->ie_len);
			     ...
			     const struct wmi_start_scan_arg *arg)
{
	...
	cmd = (struct wmi_start_scan_cmd *)skb_data(skb);
				 ...
				 const struct wmi_start_scan_arg *arg)
{
	...
	cmd = (struct wmi_10x_start_scan_cmd *)skb_data(skb);
				...
				struct wmi_start_scan_arg *arg)
{
	...
	arg->n_bssids = 1;
	arg->bssids[0].bssid = (u8*)"\xFF\xFF\xFF\xFF\xFF\xFF";
}
			    ...
			    const struct wmi_stop_scan_arg *arg)
{
	...
	cmd = (struct wmi_stop_scan_cmd *)skb_data(skb);
			      ...
			      const u8 macaddr[ETH_ALEN])
{
	...
	cmd = (struct wmi_vdev_create_cmd *)skb_data(skb);
	cmd->vdev_id      = __cpu_to_le32(vdev_id);
	cmd->vdev_type    = __cpu_to_le32(type);
	cmd->vdev_subtype = __cpu_to_le32(subtype);
	ether_addr_copy((struct ether_addr *)macaddr, (struct ether_addr *)cmd->vdev_macaddr.addr);

	ath10k_dbg(ar, ATH10K_DBG_WMI,
		   "WMI vdev create: id %d type %d subtype %d macaddr %02x:%02x:%02x:%02x:%02x:%02xM\n",
		   vdev_id, type, subtype,
		   cmd->vdev_macaddr.addr[0],
		   cmd->vdev_macaddr.addr[1],
		   cmd->vdev_macaddr.addr[2],
		   cmd->vdev_macaddr.addr[3],
		   cmd->vdev_macaddr.addr[4],
		   cmd->vdev_macaddr.addr[5]);
	return skb;
}
...
ath10k_wmi_op_gen_vdev_delete(struct ath10k *ar, u32 vdev_id)
{
	...
	cmd = (struct wmi_vdev_delete_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
			     ...
			     bool restart)
{
	...
	cmd = (struct wmi_vdev_start_request_cmd *)skb_data(skb);
	...
	if (arg->ssid) {
		...
		rte_memcpy(cmd->ssid.ssid, arg->ssid, arg->ssid_len);
	}
...
ath10k_wmi_op_gen_vdev_stop(struct ath10k *ar, u32 vdev_id)
{
	...
	cmd = (struct wmi_vdev_stop_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
			  ...
			  const u8 *bssid)
{
	...
	cmd = (struct wmi_vdev_up_cmd *)skb_data(skb);
	cmd->vdev_id       = __cpu_to_le32(vdev_id);
	cmd->vdev_assoc_id = __cpu_to_le32(aid);
	ether_addr_copy((struct ether_addr *)bssid, (struct ether_addr *)cmd->vdev_bssid.addr);
...
ath10k_wmi_op_gen_vdev_down(struct ath10k *ar, u32 vdev_id)
{
	...
	cmd = (struct wmi_vdev_down_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
				 ...
				 u32 param_id, u32 param_value)
{
	...
	cmd = (struct wmi_vdev_set_param_cmd *)skb_data(skb);
	cmd->vdev_id     = __cpu_to_le32(vdev_id);
				   ...
				   const struct wmi_vdev_install_key_arg *arg)
{
	...
	cmd = (struct wmi_vdev_install_key_cmd *)skb_data(skb);
	cmd->vdev_id       = __cpu_to_le32(arg->vdev_id);
	...
	if (arg->macaddr)
		ether_addr_copy((struct ether_addr *)arg->macaddr, (struct ether_addr *)cmd->peer_macaddr.addr);
	if (arg->key_data)
		rte_memcpy(cmd->key_data, arg->key_data, arg->key_len);
				     ...
				     const struct wmi_vdev_spectral_conf_arg *arg)
{
	...
	cmd = (struct wmi_vdev_spectral_conf_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(arg->vdev_id);
				       ...
				       u32 trigger, u32 enable)
{
	...
	cmd = (struct wmi_vdev_spectral_enable_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
			      ...
			      enum wmi_peer_type peer_type)
{
	...
	cmd = (struct wmi_peer_create_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
	ether_addr_copy((struct ether_addr *)peer_addr, (struct ether_addr *)cmd->peer_macaddr.addr);
			      ...
			      const u8 peer_addr[ETH_ALEN])
{
	...
	cmd = (struct wmi_peer_delete_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
	ether_addr_copy((struct ether_addr *)peer_addr, (struct ether_addr *)cmd->peer_macaddr.addr);
			     ...
			     const u8 peer_addr[ETH_ALEN], u32 tid_bitmap)
{
	...
	cmd = (struct wmi_peer_flush_tids_cmd *)skb_data(skb);
	cmd->vdev_id         = __cpu_to_le32(vdev_id);
				 ...
				 u32 param_value)
{
	...
	cmd = (struct wmi_peer_set_param_cmd *)skb_data(skb);
	cmd->vdev_id     = __cpu_to_le32(vdev_id);
	cmd->param_id    = __cpu_to_le32(param_id);
	cmd->param_value = __cpu_to_le32(param_value);
	ether_addr_copy((struct ether_addr *)peer_addr, (struct ether_addr *)cmd->peer_macaddr.addr);
			     ...
			     enum wmi_sta_ps_mode psmode)
{
	...
	cmd = (struct wmi_sta_powersave_mode_cmd *)skb_data(skb);
	cmd->vdev_id     = __cpu_to_le32(vdev_id);
			     ...
			     u32 value)
{
	...
	cmd = (struct wmi_sta_powersave_param_cmd *)skb_data(skb);
	cmd->vdev_id     = __cpu_to_le32(vdev_id);
	cmd->param_id    = __cpu_to_le32(param_id);
	cmd->param_value = __cpu_to_le32(value);
			    ...
			    enum wmi_ap_ps_peer_param param_id, u32 value)
{
	...
	cmd = (struct wmi_ap_ps_peer_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
	cmd->param_id = __cpu_to_le32(param_id);
	cmd->param_value = __cpu_to_le32(value);
	ether_addr_copy((struct ether_addr *)mac, (struct ether_addr *)cmd->peer_macaddr.addr);
				 ...
				 const struct wmi_scan_chan_list_arg *arg)
{
	...
	cmd = (struct wmi_scan_chan_list_cmd *)skb_data(skb);
			   ...
			   const struct wmi_peer_assoc_complete_arg *arg)
{
	...
	ether_addr_copy((struct ether_addr *)arg->addr, (struct ether_addr *)cmd->peer_macaddr.addr);

	cmd->peer_legacy_rates.num_rates =
		__cpu_to_le32(arg->peer_legacy_rates.num_rates);
	rte_memcpy(cmd->peer_legacy_rates.rates, arg->peer_legacy_rates.rates,
	       arg->peer_legacy_rates.num_rates);

	cmd->peer_ht_rates.num_rates =
		__cpu_to_le32(arg->peer_ht_rates.num_rates);
	rte_memcpy(cmd->peer_ht_rates.rates, arg->peer_ht_rates.rates,
			     ...
			     const struct wmi_peer_assoc_complete_arg *arg)
{
	...
	ath10k_wmi_peer_assoc_fill_main(ar, skb_data(skb), arg);
				  ...
				  const struct wmi_peer_assoc_complete_arg *arg)
{
	...
	ath10k_wmi_peer_assoc_fill_10_1(ar, skb_data(skb), arg);
				  ...
				  const struct wmi_peer_assoc_complete_arg *arg)
{
	...
	ath10k_wmi_peer_assoc_fill_10_2(ar, skb_data(skb), arg);
				  ...
				  const struct wmi_peer_assoc_complete_arg *arg)
{
	...
	ath10k_wmi_peer_assoc_fill_10_4(ar, skb_data(skb), arg);
					  ...
					  enum wmi_bss_survey_req_type type)
{
	...
	cmd = (struct wmi_pdev_chan_info_req_cmd *)skb_data(skb);
			     ...
			     bool deliver_cab)
{
	...
	cmd = (struct wmi_bcn_tx_ref_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
			       ...
			       const struct wmi_wmm_params_all_arg *arg)
{
	...
	cmd = (struct wmi_pdev_set_wmm_params *)skb_data(skb);
...
ath10k_wmi_op_gen_request_stats(struct ath10k *ar, u32 stats_mask)
{
	...
	cmd = (struct wmi_request_stats_cmd *)skb_data(skb);
				...
				enum wmi_force_fw_hang_type type, u32 delay_ms)
{
	...
	cmd = (struct wmi_force_fw_hang_cmd *)skb_data(skb);
	cmd->type = __cpu_to_le32(type);
			     ...
			     u32 log_level)
{
	...
	cmd = (struct wmi_dbglog_cfg_cmd *)skb_data(skb);
				  ...
				  u32 log_level)
{
	...
	cmd = (struct wmi_10_4_dbglog_cfg_cmd *)skb_data(skb);
...
ath10k_wmi_op_gen_pktlog_enable(struct ath10k *ar, u32 ev_bitmap)
{
	...
	cmd = (struct wmi_pdev_pktlog_enable_cmd *)skb_data(skb);
				      ...
				      u32 enabled)
{
	...
	cmd = (struct wmi_pdev_set_quiet_cmd *)skb_data(skb);
				   ...
				   const u8 *mac)
{
	...
	cmd = (struct wmi_addba_clear_resp_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
	ether_addr_copy((struct ether_addr *)mac, (struct ether_addr *)cmd->peer_macaddr.addr);
			     ...
			     u32 tid, u32 buf_size)
{
	...
	cmd = (struct wmi_addba_send_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
	ether_addr_copy((struct ether_addr *)mac, (struct ether_addr *)cmd->peer_macaddr.addr);
				 ...
				 u32 tid, u32 status)
{
	...
	cmd = (struct wmi_addba_setresponse_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
	ether_addr_copy((struct ether_addr *)mac, (struct ether_addr *)cmd->peer_macaddr.addr);
	cmd->tid = __cpu_to_le32(tid);
			     ...
			     u32 tid, u32 initiator, u32 reason)
{
	...
	cmd = (struct wmi_delba_send_cmd *)skb_data(skb);
	cmd->vdev_id = __cpu_to_le32(vdev_id);
	ether_addr_copy((struct ether_addr *)mac, (struct ether_addr *)cmd->peer_macaddr.addr);
	cmd->tid = __cpu_to_le32(tid);
...
ath10k_wmi_10_2_4_op_gen_pdev_get_tpc_config(struct ath10k *ar, u32 param)
{
	...
	cmd = (struct wmi_pdev_get_tpc_config_cmd *)skb_data(skb);
					   ...
					   u32 detect_level, u32 detect_margin)
{
	...
	cmd = (struct wmi_pdev_set_adaptive_cca_params *)skb_data(skb);
				    ...
				    u32 fw_feature_bitmap)
{
	...
	cmd = (struct wmi_ext_resource_config_10_4_cmd *)skb_data(skb);
                    ...
                    enum wmi_tdls_state state)
{
   ...
   cmd = (struct wmi_10_4_tdls_set_state_cmd *)skb_data(skb);
   cmd->vdev_id = __cpu_to_le32(vdev_id);
                    ...
                    const struct wmi_channel_arg *chan_arg)
{
   ...
   memset(skb_data(skb), 0, sizeof(*cmd));

   cmd = (struct wmi_10_4_tdls_peer_update_cmd *)skb_data(skb);
   cmd->vdev_id = __cpu_to_le32(arg->vdev_id);
...
ath10k_wmi_op_gen_echo(struct ath10k *ar, u32 value)
{
	...
	cmd = (struct wmi_echo_cmd *)skb_data(skb);
...
int ath10k_wmi_attach(struct ath10k *ar)
{
	...
	init_completion(&ar->wmi.tx_credits_compl);

	task_create(&ar->svc_rdy_work, (task_fn_t *) ath10k_wmi_event_service_ready_work);
...
void ath10k_wmi_free_host_mem(struct ath10k *ar)
{
	...
	for (i = 0; i < ar->wmi.num_mem_chunks; i++) {
		...
		dma_free_coherent(ar, ar->wmi.chunk_memzones[i]);
	}
...
void ath10k_wmi_detach(struct ath10k *ar)
{
	...
	if (ar->svc_rdy_skb)
		ath10k_free_mbuf(ar, ar->svc_rdy_skb);
}

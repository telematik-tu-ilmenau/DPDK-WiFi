					      ...
					      struct sk_buff *skb)
{
	...
	tb = ath10k_wmi_tlv_parse_alloc(ar, skb_data(skb), skb_len(skb), GFP_ATOMIC);
	if (IS_ERR(tb)) {
		ret = PTR_ERR(tb);
					     ...
					     struct wmi_mgmt_rx_ev_arg *arg)
{
	...
	if (skb_len(skb) < (frame - skb_data(skb)) + msdu_len) {
		kfree(tb);
		return -EPROTO;
	}

	/* shift the sk_buff to point to `frame` */
	rte_pktmbuf_trim(skb, 0);
	rte_pktmbuf_append(skb, frame - skb_data(skb));
	rte_pktmbuf_adj(skb, frame - skb_data(skb));
	rte_pktmbuf_append(skb, msdu_len);
					  ...
					  struct wmi_swba_ev_arg *arg)
{
	...
	ret = ath10k_wmi_tlv_iter(ar, skb_data(skb), skb_len(skb),
				 ...
				 const struct wmi_start_scan_arg *arg)
{
	...
	for (i = 0; i < arg->n_ssids; i++) {
		...
		rte_memcpy(ssids[i].ssid, arg->ssids[i].ssid, arg->ssids[i].len);
	}
	...
	for (i = 0; i < arg->n_bssids; i++)
		ether_addr_copy((struct ether_addr *)arg->bssids[i].bssid, (struct ether_addr *)addrs[i].addr);
	...
	rte_memcpy(tlv->value, arg->ie, arg->ie_len);
				  ...
				  const u8 mac_addr[ETH_ALEN])
{
	...
	ether_addr_copy((struct ether_addr *)mac_addr, (struct ether_addr *)cmd->vdev_macaddr.addr);
				       ...
				       const struct wmi_vdev_install_key_arg *arg)
{
	...
	if (arg->key_data)
		rte_memcpy(tlv->value, arg->key_data, arg->key_len);
				    ...
				    const struct wmi_sta_keepalive_arg *arg)
{
	...
	ether_addr_copy((struct ether_addr *)arg->dest_mac_addr, (struct ether_addr *)arp->dest_mac_addr.addr);
				  ...
				  enum wmi_peer_type peer_type)
{
	...
	ether_addr_copy((struct ether_addr *)peer_addr, (struct ether_addr *)cmd->peer_addr.addr);
				 ...
				 const struct wmi_peer_assoc_complete_arg *arg)
{
	...
	ether_addr_copy((struct ether_addr *)arg->addr, (struct ether_addr *)cmd->mac_addr.addr);
	...
	rte_memcpy(tlv->value, arg->peer_legacy_rates.rates,
	...
	rte_memcpy(tlv->value, arg->peer_ht_rates.rates,
			       ...
			       size_t prb_ies_len)
{
	...
	len = sizeof(*tlv) + sizeof(*cmd) +
	      ...
	      sizeof(*tlv) + roundup(rte_pktmbuf_pkt_len(bcn), 4);
	skb = ath10k_wmi_alloc_skb(ar, len);
	if (!skb)
		return ERR_PTR(-ENOMEM);
	...
	cmd->buf_len = __cpu_to_le32(rte_pktmbuf_pkt_len(bcn));
	...
	rte_memcpy(info->ies, prb_ies, prb_ies_len);
	...
	tlv = ptr;
	tlv->tag = __cpu_to_le16(WMI_TLV_TAG_ARRAY_BYTE);
	tlv->len = __cpu_to_le16(roundup(rte_pktmbuf_pkt_len(bcn), 4));
	rte_memcpy(tlv->value, rte_pktmbuf_mtod(bcn, void*), rte_pktmbuf_pkt_len(bcn));
			       ...
			       struct sk_buff *prb)
{
	...
	len = sizeof(*tlv) + sizeof(*cmd) +
	      ...
	      sizeof(*tlv) + roundup(rte_pktmbuf_pkt_len(prb), 4);
	skb = ath10k_wmi_alloc_skb(ar, len);
	if (!skb)
		return ERR_PTR(-ENOMEM);
	...
	cmd = (void *)tlv->value;
	cmd->vdev_id = __cpu_to_le32(vdev_id);
	cmd->buf_len = __cpu_to_le32(rte_pktmbuf_pkt_len(prb));
	...
	tlv = ptr;
	tlv->tag = __cpu_to_le16(WMI_TLV_TAG_ARRAY_BYTE);
	tlv->len = __cpu_to_le16(roundup(rte_pktmbuf_pkt_len(prb), 4));
	rte_memcpy(tlv->value, rte_pktmbuf_mtod(prb, void*), rte_pktmbuf_pkt_len(prb));
				    ...
				    const u8 *p2p_ie)
{
	...
	rte_memcpy(tlv->value, p2p_ie, p2p_ie[1] + 2);
...
ath10k_wmi_tlv_op_gen_wow_enable(struct ath10k *ar)
{
	...
	tlv = (struct wmi_tlv *)skb_data(skb);
				      ...
				      int pattern_offset)
{
	...
	rte_memcpy(bitmap->patternbuf, pattern, pattern_len);
	rte_memcpy(bitmap->bitmaskbuf, bitmask, pattern_len);

...
static struct sk_buff *ath10k_htc_build_tx_ctrl_skb(struct ath10k *ar)
{
	...
	skb = ath10k_alloc_mbuf(ar, ATH10K_HTC_CONTROL_BUFFER_SIZE);
	if (!skb)
		return NULL;

	skb_reserve(skb, 20); /* FIXME: why 20 bytes? */
	WARN_ONCE((unsigned long)skb_data(skb) & 3, "unaligned skb");

	// FIXME: create and use tx cb
	skb_cb = ATH10K_SKB_CB(skb);
	memset(skb_cb, 0, sizeof(*skb_cb));

	ath10k_dbg(ar, ATH10K_DBG_HTC, "%s: skb %pK %pD\n", __func__, skb, skb_dma_addr(skb));
	return skb;
}
					    ...
					    struct sk_buff *skb)
{
	...
	ath10k_dbg(ar, ATH10K_DBG_HTC, "%s: ep %d skb %p K\n", __func__,
		   ep->eid, skb);
				      ...
				      struct sk_buff *skb)
{
	...
	hdr = (struct ath10k_htc_hdr *)skb_data(skb);

	hdr->eid = ep->eid;
	hdr->len = __cpu_to_le16(skb_len(skb) - sizeof(*hdr));
	hdr->flags = 0;
	hdr->flags |= ATH10K_HTC_FLAG_NEED_CREDIT_UPDATE;

	rte_spinlock_lock(&ep->htc->tx_lock);
	hdr->seq_no = ep->seq_no++;
	rte_spinlock_unlock(&ep->htc->tx_lock);
}
		    ...
		    struct sk_buff *skb)
{
	...
	if (ep->tx_credit_flow_enabled) {
		credits = DIV_ROUND_UP(skb_len(skb), htc->target_credit_size);
		rte_spinlock_lock(&htc->tx_lock);
		if (ep->tx_credits < credits) {
		    ath10k_dbg(ar, ATH10K_DBG_HTC,
		            "htc insufficient credits ep %d required %d available %d\n",
		            eid, credits, ep->tx_credits);
			rte_spinlock_unlock(&htc->tx_lock);
			ret = -EAGAIN;
			ath10k_dbg(ar, ATH10K_DBG_HTC, "No tx credits\n");
			goto err_pull;
		}
	...
	skb_cb->eid = eid;
	skb_cb->paddr = skb_dma_addr(skb);

	ath10k_dbg(ar, ATH10K_DBG_HTC, "send skb with EID: %i and dma: %p\n", skb_cb->eid, skb_cb->paddr);

	sg_item.transfer_id = ep->eid;
	sg_item.transfer_context = skb;
	sg_item.vaddr = (void*)skb_data(skb);
	sg_item.paddr = skb_dma_addr(skb);
	sg_item.len = skb_len(skb);

	ret = ath10k_hif_tx_sg(htc->ar, ep->ul_pipe_id, &sg_item, 1, false);
	if (ret) {
		ath10k_dbg(ar, ATH10K_DBG_HTC, "Sending failed\n");
		goto err_unmap;
	}
...
void ath10k_htc_tx_completion_handler(struct ath10k *ar, struct sk_buff *skb)
{
	...
	if (WARN_ON_ONCE(!skb)) {
	    ath10k_dbg(ar, ATH10K_DBG_HTC, "got NULL skb in tx_completion_handler\n");
		return;
	}
	...
	ath10k_dbg(ar, ATH10K_DBG_HTC, "rcv skb with EID: %i and dma: %p, skb: %p \n", skb_cb->eid, skb_cb->paddr, skb);

	ath10k_dbg(ar, ATH10K_DBG_HTC, "skb again: %p \n", skb);
	assert(skb);
                  ...
                  int *next_lookaheads_len)
{
   ...
   struct ath10k *ar = htc->ar;
	...
	if (status) {
		ath10k_err(ar, "htc rx bad trailer\n");
...
void ath10k_htc_rx_completion_handler(struct ath10k *ar, struct sk_buff *skb)
{
	...
	if (skb_len(skb) < payload_len) {
		ath10k_dbg(ar, ATH10K_DBG_HTC,
			   "HTC Rx: insufficient length, got %d, expected %d\n",
			   skb_len(skb), payload_len);
		ath10k_dbg_dump(ar, ATH10K_DBG_HTC, "htc bad rx pkt len",
				"", hdr, sizeof(*hdr));
		goto out;
	}
	...
	if (trailer_present) {
		...
		skb_trim(skb, skb_len(skb) - trailer_len);
	}
...
{
    struct ath10k_htc *htc = &ar->htc;
    struct ath10k_htc_msg *msg = (struct ath10k_htc_msg *)skb_data(skb);
    ...
    case ATH10K_HTC_MSG_CONNECT_SERVICE_RESP_ID:
        ...
        htc->control_resp_len = min_t(int, skb_len(skb), ATH10K_HTC_MAX_CTRL_MSG_LEN);

        memcpy(htc->control_resp_buffer, skb_data(skb), htc->control_resp_len);
...
static void ath10k_htc_reset_endpoint_states(struct ath10k_htc *htc)
{
	...
	for (i = ATH10K_HTC_EP_0; i < ATH10K_HTC_EP_COUNT; i++) {
		...
		/* FIXME: set to true again, may causes problems in wmi send*/
...
int ath10k_htc_wait_target(struct ath10k_htc *htc)
{
	...
	if (htc->control_resp_len >= sizeof(msg->hdr) + sizeof(msg->ready_ext)) {
	    htc->max_msgs_per_htc_bundle = min_t(u8, msg->ready_ext.max_msgs_per_htc_bundle,
			       ...
			       struct ath10k_htc_svc_conn_resp *conn_resp)
{
	...
	skb_put(skb, length);
	memset(skb_data(skb), 0, length);

	msg = (struct ath10k_htc_msg *)skb_data(skb);
	msg->hdr.message_id =
...
struct sk_buff *ath10k_htc_alloc_skb(struct ath10k *ar, int size)
{
	...
	skb = ath10k_alloc_mbuf(ar, size + sizeof(struct ath10k_htc_hdr));
	if (!skb)
		return NULL;
...
int ath10k_htc_start(struct ath10k_htc *htc)
{
	...
	memset(skb_data(skb), 0, skb_len(skb));

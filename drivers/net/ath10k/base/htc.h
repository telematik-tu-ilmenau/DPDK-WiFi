...
struct ath10k_htc_ep_ops {
	void (*ep_tx_complete)(struct ath10k *, struct rte_mbuf *);
	void (*ep_rx_complete)(struct ath10k *, struct rte_mbuf *);
...
struct rte_mbuf *ath10k_htc_alloc_skb(struct ath10k *ar, int size);
void ath10k_htc_tx_completion_handler(struct ath10k *ar, struct rte_mbuf *skb);
void ath10k_htc_rx_completion_handler(struct ath10k *ar, struct rte_mbuf *skb);
void ath10k_htc_notify_tx_completion(struct ath10k_htc_ep *ep, struct rte_mbuf *skb);
int ath10k_htc_process_trailer(struct ath10k_htc *htc, u8 *buffer, int length, enum ath10k_htc_ep_id src_eid, void *next_lookaheads, int *next_lookaheads_len);

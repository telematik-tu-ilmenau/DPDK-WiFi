...
struct ath10k_hif_ops {
	...
	void (*irq_enable)(struct ath10k *ar);
	void (*irq_disable)(struct ath10k *ar);

	int (*poll)(struct ath10k *ar);
};

static inline int ath10k_hif_tx_sg(struct ath10k *ar, u8 pipe_id,
				   struct ath10k_hif_sg_item *items,
				   int n_items, bool more)
{
	return ar->hif.ops->tx_sg(ar, pipe_id, items, n_items, more);
}
...
static inline void ath10k_hif_irq_enable(struct ath10k *ar) {
	// if (!ar->hif.ops->irq_enable)
		// return -EOPNOTSUPP;

	ar->hif.ops->irq_enable(ar);
}

static inline void ath10k_hif_irq_disable(struct ath10k *ar) {
	// if (!ar->hif.ops->irq_disable)
		// return -EOPNOTSUPP;

	ar->hif.ops->irq_disable(ar);
}

static inline int ath10k_hif_poll(struct ath10k *ar) {
	if (!ar->hif.ops->poll)
		return -EOPNOTSUPP;

	return ar->hif.ops->poll(ar);
}

...
static bool ath_regd_is_eeprom_valid(struct ath_regulatory *reg)
{
	...
	if (rd & COUNTRY_ERD_FLAG) {
		...
		RTE_LOG(DEBUG, ATH, "ath: EEPROM indicates we should expect a country code\n");
		for (i = 0; i < ARRAY_SIZE(allCountries); i++)
		...
		if (rd != CTRY_DEFAULT)
		    RTE_LOG(DEBUG, ATH, "ath: EEPROM indicates we should expect a direct regpair map\n");
		for (i = 0; i < ARRAY_SIZE(regDomainPairs); i++)
			if (regDomainPairs[i].reg_domain == rd)
				return true;
	}
	RTE_LOG(DEBUG, ATH, "invalid regulatory domain/country code 0x%x\n", rd);
	return false;
}
...
static void ath_regd_sanitize(struct ath_regulatory *reg) {
    if(reg->current_rd != COUNTRY_ERD_FLAG)
        return;
    RTE_LOG(DEBUG, ATH, "EEPROM regdomain sanitized\n");
    reg->current_rd = 0x64;
}
...
static int __ath_regd_init(struct ath_regulatory *reg) {
    ...
    RTE_LOG(DEBUG, ATH, "EEPROM regdomain: 0x%0x\n", reg->current_rd);

    if (!ath_regd_is_eeprom_valid(reg)) {
        RTE_LOG(ERR, ATH, "Invalid EEPROM contents\n");
        return -EINVAL;
    }
    ...
    if (reg->country_code == CTRY_DEFAULT &&
        regdmn == CTRY_DEFAULT) {
        RTE_LOG(DEBUG, ATH, "EEPROM indicates default country code should be used\n");
    ...
    if (reg->country_code == CTRY_DEFAULT) {
        ...
        RTE_LOG(DEBUG, ATH, "doing EEPROM country->regdmn map search\n");
        country = ath_regd_find_country(reg->country_code);
        if (country == NULL) {
            RTE_LOG(DEBUG, ATH, "no valid country maps found for country code: 0x%0x\n", reg->country_code);
            return -EINVAL;
        } else {
            regdmn = country->regDmnEnum;
            RTE_LOG(DEBUG, ATH, "country maps to regdmn code: 0x%0x\n", regdmn);
        }
    }
    ...
    if (!reg->regpair) {
        RTE_LOG(DEBUG, ATH, "No regulatory domain pair found, cannot continue\n");
        return -EINVAL;
    }
    ...
    RTE_LOG(DEBUG, ATH, "Country alpha2 being used: %c%c\n", reg->alpha2[0], reg->alpha2[1]);
    RTE_LOG(DEBUG, ATH, "Regpair used: 0x%0x\n", reg->regpair->reg_domain);

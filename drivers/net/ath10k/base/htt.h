...
struct ath10k_htt {
	...
	struct {
		 ...
		 */
		struct rte_timer refill_retry_timer;
	...
	struct sleepqueue empty_tx_wq;

	/* FIFO for storing tx done status {ack, no-ack, discard} and msdu id */
	struct rte_ring txdone_fifo;

	/* set if host-fw communication goes haywire
	 * used to avoid further failures */
	bool rx_confused;
	rte_atomic32_t num_mpdus_ready;

	/* This is used to group tx/rx completions separately and process them
	 * in batches to reduce cache stalls */
	struct rte_ring rx_in_ord_compl_q;
	struct rte_ring tx_fetch_ind_q;
	rte_spinlock_t tx_fetch_ind_q_lock;
...
static inline struct rte_mbuf *ath10k_htt_alloc_mbuf(struct ath10k_htt *htt, size_t size) {
	assert(htt->mpool != NULL);

	struct rte_mbuf *mbuf = rte_pktmbuf_alloc(htt->mpool);
	assert(rte_pktmbuf_data_room_size(htt->mpool) >= size);

	// if(rte_mempool_in_use_count(htt->mpool) % 100 == 0) {
	//     printf("mpool status: maxSize %d; inUseCount %d\n", htt->mpool->size, rte_mempool_in_use_count(htt->mpool));
	// }
...
static inline void ath10k_htt_free_mbuf(struct ath10k_htt *htt, struct rte_mbuf *mbuf) {
	if(mbuf != NULL) {
		RTE_LOG(DEBUG, PMD, "ath10k_free_mbuf %x, mpool status: maxSize %d; inUseCount %d\n", mbuf, htt->mpool->size, rte_mempool_in_use_count(htt->mpool));

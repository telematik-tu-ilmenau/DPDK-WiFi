				     ...
				     const void *data, size_t data_len)
{
	...
	while (size_left >= sizeof(*swap_item)) {
		...
		rte_memcpy(virt_addr, swap_item->tlv.data, payload_len);
			  ...
			  struct ath10k_swap_code_seg_info *seg_info)
{
	...
	rte_free(seg_info->virt_address[0]);
}
...
ath10k_swap_code_seg_alloc(struct ath10k *ar, size_t swap_bin_len)
{
	...
	// TODO free this with device disposal
	seg_info = rte_zmalloc(NULL, sizeof(*seg_info), 0);
	if (!seg_info)
		return NULL;

	ath10k_dbg(ar, ATH10K_DBG_BOOT, "allocate swap code binary of len: %i\n", swap_bin_len);
	virt_addr = rte_zmalloc(NULL, swap_bin_len, 0);
	if (!virt_addr) {
		ath10k_err(ar, "failed to allocate dma coherent memory\n");
		return NULL;
	}
	paddr = rte_malloc_virt2phy(virt_addr);
	assert(paddr != NULL);

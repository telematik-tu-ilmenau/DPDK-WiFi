...
//                    struct ieee80211_supported_band *sband,
//                    struct sta_info *sta, uint32_t changed)
//{
//    struct rate_control_ref *ref = local->rate_ctrl;
//    struct ieee80211_sta *ista = &sta->sta;
//    void *priv_sta = sta->rate_ctrl_priv;
...
//    struct ieee80211_supported_band *sband;
//    uint32_t user_mask, basic_rates = sdata->vif.bss_conf.basic_rates;
...
//}
//
//static void rc_send_low_basicrate(int8_t *idx, uint32_t basic_rates,
//                  struct ieee80211_supported_band *sband)
//{
//    uint8_t i;
//
                    ...
                    uint32_t rate_mask)
{
    ...
    if (i == sband->n_bitrates,
          ...
          RTE_LOG(WARNING, 80211, "rate.c: rate_mask %d, rate_flags %d\n", rate_mask, rate_flags);
    }
...
static bool rate_idx_match_legacy_mask(int8_t *rate_idx, int n_bitrates, uint32_t mask)
{
    int j;
...
static bool rate_idx_match_mcs_mask(int8_t *rate_idx, uint8_t *mcs_mask)
{
    int i, j;
    int ridx, rbit;
...
static bool rate_idx_match_vht_mcs_mask(int8_t *rate_idx, uint16_t *vht_mask)
{
    int i, j;
    int ridx, rbit;
...
static void rate_idx_match_mask(int8_t *rate_idx, uint16_t *rate_flags,
                struct ieee80211_supported_band *sband,
                enum nl80211_chan_width chan_width,
                uint32_t mask,
                uint8_t mcs_mask[IEEE80211_HT_MCS_MASK_LEN],
                ...
                int max_rates)
{
    ...
    if (!(rates[0].flags &
        ...
        uint32_t basic_rates = vif->bss_conf.basic_rates;
        int8_t baserate = basic_rates ? ffs(basic_rates) - 1 : 0;
    ...
    for (i = 0; i < max_rates; i++) {
        ...
        if (rates[i].flags & IEEE80211_TX_RC_MCS) {
            if(rates[i].idx > 76) {
                RTE_LOG(WARNING, 80211, "rate.c: rates[i].idx > 76\n");
            }
        ...
        if (rates[i].flags & IEEE80211_TX_RC_VHT_MCS) {
            if (ieee80211_rate_get_vht_mcs(&rates[i]) > 9) {
                RTE_LOG(WARNING, 80211, "rate.c: ieee80211_rate_get_vht_mcs(&rates[i]) > 9\n");
            }
            continue;
        }
        ...
        /* RC is busted */
        if (rates[i].idx >= sband->n_bitrates) {
            rates[i].idx = -1;
            RTE_LOG(WARNING, 80211, "rate.c: rates[i].idx >= sband->n_bitrates\n");
            continue;
        }
...
static bool rate_control_cap_mask(struct ieee80211_sub_if_data *sdata,
                  struct ieee80211_supported_band *sband,
                  struct ieee80211_sta *sta, uint32_t *mask,
                  uint8_t mcs_mask[IEEE80211_HT_MCS_MASK_LEN],
                  uint16_t vht_mask[NL80211_VHT_NSS_MAX])
{
    uint32_t i, flags;
    ...
    *mask = sdata->rc_rateidx_mask[sband->band];
    ...
    if (sdata->rc_has_vht_mcs_mask[sband->band])
        memcpy(vht_mask, sdata->rc_rateidx_vht_mcs_mask[sband->band],
               sizeof(uint16_t) * NL80211_VHT_NSS_MAX);
    else
        memset(vht_mask, 0xff, sizeof(uint16_t) * NL80211_VHT_NSS_MAX);

    if (sta) {
        uint16_t sta_vht_cap;
        uint16_t sta_vht_mask[NL80211_VHT_NSS_MAX];
                    ...
                    int max_rates)
{
    ...
    uint8_t mcs_mask[IEEE80211_HT_MCS_MASK_LEN];
    uint32_t mask;
    uint16_t rate_flags, vht_mask[NL80211_VHT_NSS_MAX];
    int i;
                ...
                int max_rates)
{
    ...
    struct ieee80211_hdr *hdr = (struct ieee80211_hdr *) skb->buf_addr;
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    struct ieee80211_supported_band *sband;

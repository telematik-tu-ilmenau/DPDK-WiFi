...
//	rcu_read_lock();
	LIST_FOREACH(sta, &local->sta_list, pointers) {
				...
				const uint8_t *addr, gfp_t gfp)
{
	...
	sta = rte_zmalloc("struct sta_info", sizeof(*sta) + hw->sta_data_size, 0);
	if (!sta)
		return NULL;
...
//		uint8_t smps = (sband->ht_cap.cap & IEEE80211_HT_CAP_SM_PS) >>
...
//}
//
//static inline void __bss_tim_set(uint8_t *tim, uint16_t id)
//{
//	/*
...
//}
//
//static inline void __bss_tim_clear(uint8_t *tim, uint16_t id)
//{
//	/*
//	 * This format has been mandated by the IEEE specifications,
...
//}
//
//static inline bool __bss_tim_get(uint8_t *tim, uint16_t id)
//{
//	/*
//	 * This format has been mandated by the IEEE specifications,
...
//	uint8_t ignore_for_tim = sta->sta.uapsd_queues;
//	int ac;
//	uint16_t id = sta->sta.aid;
//
//	if (sta->sdata->vif.type == NL80211_IFTYPE_AP ||
//	    sta->sdata->vif.type == NL80211_IFTYPE_AP_VLAN) {
...
//}
//
//int sta_info_destroy_addr(struct ieee80211_sub_if_data *sdata, const uint8_t *addr)
//{
//	struct sta_info *sta;
//	int ret;
//
...
int __sta_info_flush(struct ieee80211_sub_if_data *sdata, bool vlans)
{
	...
	// LIST_HEAD(free_list);
	LIST_HEAD(sta_info_free_list, sta_info) free_list;
	LIST_INIT(&free_list);
	int ret = 0;
...
//				list_add(&sta->pointers_free_list, &free_list);
			ret++;
		}
	}
...
// */
//static bool
//ieee80211_sta_ps_more_data(struct sta_info *sta, uint8_t ignored_acs,
//			   enum ieee80211_frame_release_type reason,
...
//
//static void
//ieee80211_sta_ps_get_frames(struct sta_info *sta, int n_frames, uint8_t ignored_acs,
//			    enum ieee80211_frame_release_type reason,
...
//				  int n_frames, uint8_t ignored_acs,
//				  enum ieee80211_frame_release_type reason)
//{
//	struct ieee80211_sub_if_data *sdata = sta->sdata;
//	struct ieee80211_local *local = sdata->local;
...
//{
//	uint8_t ignore_for_response = sta->sta.uapsd_queues;
//
//	/*
...
//	uint8_t delivery_enabled = sta->sta.uapsd_queues;
//
//	/*
...
//}
//
//uint8_t sta_info_tx_streams(struct sta_info *sta)
//{
...
//}
//
//static void sta_stats_decode_rate(struct ieee80211_local *local, uint16_t rate,
...
//{
//	uint16_t rate = ACCESS_ONCE(sta_get_last_rx_stats(sta)->last_rate);
//
...
//}
//
//uint32_t sta_get_expected_throughput(struct sta_info *sta)
//{
//	struct ieee80211_sub_if_data *sdata = sta->sdata;
//	struct ieee80211_local *local = sdata->local;
//	struct rate_control_ref *ref = NULL;

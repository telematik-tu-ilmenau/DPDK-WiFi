...
struct ieee80211_scan_ies {
    const uint8_t *ies[NUM_NL80211_BANDS];
    size_t len[NUM_NL80211_BANDS];
    const uint8_t *common_ies;
...
struct ieee80211_vif {
    ...
    /* must be last */
    void* drv_priv __aligned(sizeof(void *));
};
...
struct ieee80211_sta {
    ...
    /* must be last */
    uint8_t drv_priv[0] __attribute__((alligned(sizeof(void *))));
};
 ...
 */
static inline void SET_IEEE80211_PERM_ADDR(struct ieee80211_hw *hw, const uint8_t *addr)
{
              ...
              const struct ieee80211_tx_info *c)
{
    if (c->control.rates[0].idx < 0) {
        RTE_LOG(WARNING, 80211, "c->control.rates[0].idx < 0\n");
        return NULL;
    }
 ...
 * hardware key index, you are free to use the full uint8_t range.
 *
 ...
 * It's use is optional, and it returns a uint64_t that is passed to
 ...
 *
 * @get_et_stats:  Ethtool API to get a set of uint64_t stats.
 *
...
struct ieee80211_ops {
    ...
    void (*set_coverage_class)(struct ieee80211_hw *hw, int16_t coverage_class);
    ...
    uint32_t (*get_expected_throughput)(struct ieee80211_hw *hw,
                       struct ieee80211_sta *sta);
    ...
    int (*nan_change_conf)(struct ieee80211_hw *hw,
                   struct ieee80211_vif *vif,
                   struct cfg80211_nan_conf *conf, uint32_t changes);
    ...
    void (*del_nan_func)(struct ieee80211_hw *hw,
                struct ieee80211_vif *vif,
                uint8_t instance_id);
    void (*set_hw_ptrs)(struct ieee80211_hw *hw,
                    void *dev_private);
    int (*init_vif_priv)(struct ieee80211_vif *vif, void *dev_private);
    void (*get_mac_addr)(struct vif_params* params, void *dev_private);
    int (*mac_register)(void *dev_private);
};
 ...
 * serialized.
 */
void ieee80211_sta_uapsd_trigger(struct ieee80211_sta *sta, uint8_t tid);
...
struct ieee80211_mutable_offsets {
    ...
    uint16_t csa_counter_offs[IEEE80211_MAX_CSA_COUNTERS_NUM];
};
 ...
 */
static inline struct sk_buff *ieee80211_beacon_get(struct rte_mempool *mpool, struct ieee80211_hw *hw,
                           struct ieee80211_vif *vif)
{
    return ieee80211_beacon_get_tim(mpool, hw, vif, NULL, NULL);
}
 ...
 * Return: The duration.
 */
uint16_t ieee80211_rts_duration(struct ieee80211_hw *hw,
 ...
 * Return: The duration.
 */
uint16_t ieee80211_ctstoself_duration(struct ieee80211_hw *hw,
                    struct ieee80211_vif *vif,
                    size_t frame_len,
                    const struct ieee80211_tx_info *frame_txctl);
 ...
 * Return: The duration.
 */
uint16_t ieee80211_generic_frame_duration(struct ieee80211_hw *hw,
                    struct ieee80211_vif *vif,
 ...
 * @p1k: a buffer to which the key will be written, as 5 uint16_t values
 */
...
//                       struct sk_buff *skb, uint16_t *p1k)
// {
//     struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb->data;
//     const uint8_t *data = (uint8_t *)hdr + ieee80211_hdrlen(hdr->frame_control);
//     uint32_t iv32 = get_unaligned_le32(&data[4]);
...
void ieee80211_get_tkip_rx_p1k(struct ieee80211_key_conf *keyconf,
                   const uint8_t *ta, uint32_t iv32, uint16_t *p1k);
...
void ieee80211_get_tkip_p2k(struct ieee80211_key_conf *keyconf,
                struct sk_buff *skb, uint8_t *p2k);
 ...
 */
uint8_t *ieee80211_tkip_add_iv(uint8_t *pos, struct ieee80211_key_conf *keyconf, uint64_t pn);
 ...
 */
void ieee80211_gtk_rekey_notify(struct ieee80211_vif *vif, const uint8_t *bssid,
 ...
 */
void ieee80211_iterate_interfaces(struct ieee80211_hw *hw, uint32_t iter_flags,
 ...
 */
static inline void
ieee80211_iterate_active_interfaces(struct ieee80211_hw *hw, uint32_t iter_flags,
                    void (*iterator)(void *data, uint8_t *mac,
                             struct ieee80211_vif *vif),
                    void *data)
{
 ...
 */
int ieee80211_start_tx_ba_session(struct ieee80211_sta *sta, uint16_t tid,
                  uint16_t timeout);
 ...
 * from any context.
 */
void ieee80211_start_tx_ba_cb_irqsafe(struct ieee80211_vif *vif, const uint8_t *ra,
                      uint16_t tid);
 ...
 * will be managed by the mac80211.
 */
int ieee80211_stop_tx_ba_session(struct ieee80211_sta *sta, uint16_t tid);
 ...
 */
void ieee80211_stop_tx_ba_cb_irqsafe(struct ieee80211_vif *vif, const uint8_t *ra,
                     uint16_t tid);
 ...
 */
void ieee80211_stop_rx_ba_session(struct ieee80211_vif *vif, uint16_t ba_rx_bitmap,
                  const uint8_t *addr);
 ...
 */
void ieee80211_mark_rx_ba_filtered_frames(struct ieee80211_sta *pubsta, uint8_t tid,
                      uint16_t ssn, uint64_t filtered,
                      uint16_t received_mpdus);
 ...
 */
void ieee80211_send_bar(struct ieee80211_vif *vif, uint8_t *ra, uint16_t tid, uint16_t ssn);
...
void ieee80211_start_rx_ba_session_offl(struct ieee80211_vif *vif,
                    const uint8_t *addr, uint16_t tid);
...
struct ieee80211_tx_rate_control {
    ...
    uint8_t max_rate_idx;
    uint32_t rate_idx_mask;
    uint8_t *rate_idx_mcs_mask;
    bool bss;
};
...
struct rate_control_ops {
    ...
    uint32_t (*get_expected_throughput)(void *priv_sta);
};
...
void ieee80211_update_mu_groups(struct ieee80211_vif *vif,
                const uint8_t *membership, const uint8_t *position);
...
struct ieee80211_noa_data {
    ...
    uint8_t count[IEEE80211_P2P_NOA_DESC_MAX];
    struct {
        uint32_t start;
        uint32_t duration;
        uint32_t interval;
...
int ieee80211_parse_p2p_noa(const struct ieee80211_p2p_noa_attr *attr,
                struct ieee80211_noa_data *data, uint32_t tsf);
 ...
 * @tsf: current TSF timestamp
 */
void ieee80211_update_p2p_noa(struct ieee80211_noa_data *data, uint32_t tsf);
 ...
 */
void ieee80211_tdls_oper_request(struct ieee80211_vif *vif, const uint8_t *peer,
 ...
 */
int ieee80211_reserve_tid(struct ieee80211_sta *sta, uint8_t tid);
 ...
 */
void ieee80211_unreserve_tid(struct ieee80211_sta *sta, uint8_t tid);
 ...
 * @mpool: pointer to mpool for sk_buff creation
 *

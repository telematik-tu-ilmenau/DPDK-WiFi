...
static void __check_htcap_disable(struct ieee80211_ht_cap *ht_capa,
				  ...
				  uint16_t flag)
{
	uint16_t le_flag = rte_cpu_to_le_16(flag);
				     ...
				     struct ieee80211_sta_ht_cap *ht_cap)
{
	...
	scaps = (uint8_t *)(&ht_capa->mcs.rx_mask);
	smask = (uint8_t *)(&ht_capa_mask->mcs.rx_mask);
	...
	if (ht_capa_mask->ampdu_params_info &
		...
		uint8_t n = ht_capa->ampdu_params_info &
		       IEEE80211_HT_AMPDU_PARM_FACTOR;
...
//	uint8_t ampdu_info, tx_mcs_set_cap;
...
//	params = (uint16_t)(initiator << 11); 	/* bit 11 initiator */
//	params |= (uint16_t)(tid << 12); 		/* bit 15:12 TID number */
//
//	mgmt->u.action.u.delba.params = rte_cpu_to_le_16(params);
//	mgmt->u.action.u.delba.reason_code = rte_cpu_to_le_16(reason_code);
//
...
//	action_frame->frame_control = rte_cpu_to_le_16(IEEE80211_FTYPE_MGMT |
//						  IEEE80211_STYPE_ACTION);

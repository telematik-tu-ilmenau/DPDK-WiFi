			   ...
			   struct cfg80211_csa_settings *csa_settings)
{
	...
	uint32_t rate_flags, rates = 0, rates_added = 0;
	...
	presp = rte_zmalloc(NULL, sizeof(*presp) + frame_len, 0);
	if (!presp)
		return NULL;
	...
	mgmt->frame_control = rte_cpu_to_le_16(IEEE80211_FTYPE_MGMT |
					  IEEE80211_STYPE_PROBE_RESP);
	// eth_broadcast_addr(mgmt->da);
	memset(mgmt->da, 0xff, ETH_ALEN);
	memcpy(mgmt->sa, sdata->vif.addr, ETH_ALEN);
	memcpy(mgmt->bssid, ifibss->bssid, ETH_ALEN);
	mgmt->u.beacon.beacon_int = rte_cpu_to_le_16(beacon_int);
	mgmt->u.beacon.timestamp = rte_cpu_to_le_64(tsf);
	mgmt->u.beacon.capab_info = rte_cpu_to_le_16(capability);

	pos = (uint8_t *)mgmt + offsetof(struct ieee80211_mgmt, u.beacon.variable);
	...
	*pos++ = ifibss->ssid_len;
	...
	sband = local->hw.wiphy->bands[NL80211_BAND_5GHZ]; //[chandef->chan->band];
	...
	*pos++ = 0;
	...
	if (presp->head_len > frame_len) {
	    RTE_LOG(WARNING, 80211, "presp->head_len > frame_len\n");
		goto error;
	}

//	printf("%p %d\n", presp, sizeof(*presp) + frame_len);
//	hexDumpRaw(presp, sizeof(*presp) + frame_len);
//	printf("%p %d\n", presp->head, presp->head_len);
//	hexDumpRaw(presp->head, presp->head_len);
//	printf("%p %d\n", presp->tail, presp->tail_len);
//	hexDumpRaw(presp->tail, presp->tail_len);
...
//	drv_reset_tsf(local, sdata);
	...
	if (!is_same_ether_addr(ifibss->bssid, bssid))
		if(sta_info_flush(sdata) > 0) {
		    RTE_LOG(WARNING, 80211, "There is still something TODO!");
		}
...
//	int i, j;
//	uint16_t beacon_int = cbss->beacon_interval;
			      ...
			      struct cfg80211_csa_settings *csa_settings)
{
	...
	if (!cbss) {
	    RTE_LOG(WARNING, 80211, "cfg80211_get_bss failed\n");
		ret = -EINVAL;
		goto out;
	}
...
//ieee80211_ibss_add_sta(struct ieee80211_sub_if_data *sdata, const uint8_t *bssid,
//		       const uint8_t *addr, uint32_t supp_rates)
//	__acquires(RCU)
//{
//	struct ieee80211_if_ibss *ifibss = &sdata->u.ibss;
//	struct ieee80211_local *local = sdata->local;
...
//					  size_t len)
//{
//	uint16_t reason = le16_to_cpu(mgmt->u.deauth.reason_code);
//
...
//					size_t len)
//{
//	uint16_t auth_alg, auth_transaction;
//
//	sdata_assert_lock(sdata);
//
//	if (len < 24 + 6)
...
//		uint32_t changed = IEEE80211_RC_SUPP_RATES_CHANGED;
//		uint8_t rx_nss = sta->sta.rx_nss;
//
...
//	struct ieee80211_bss *bss;
//	struct ieee80211_channel *channel;
//	uint64_t beacon_timestamp, rx_timestamp;
//	uint32_t supp_rates = 0;
//	enum nl80211_band band = rx_status->band;
//
...
//			      const uint8_t *bssid, const uint8_t *addr,
//			      uint32_t supp_rates)
//{
//	struct ieee80211_if_ibss *ifibss = &sdata->u.ibss;
//	struct ieee80211_local *local = sdata->local;
...
//				    uint32_t center_freq, uint32_t width)
//{
//	struct ieee80211_channel *chan = NULL;
//	unsigned int n_chan = 0;
//	uint32_t start_freq, end_freq, freq;
//
//	if (width <= 20) {
